...
export const DB = {
  ...
  async markDelivered(env: Env, id: string): Promise<void> {
    await env.DATABASE.prepare(
      `UPDATE licenses SET delivered_at = ? WHERE id = ?`
    )
  ...
  async getByHash(env: Env, keyHash: string): Promise<LicenseRecord | null> {
    return env.DATABASE.prepare(
      `SELECT * FROM licenses WHERE key_hash = ? LIMIT 1`
    )
      .bind(keyHash)
      .first<LicenseRecord>();
  },

  async getByStripeCustomer(
    env: Env,
    customerId: string
  ): Promise<LicenseRecord | null> {
    return env.DATABASE.prepare(
      `SELECT * FROM licenses WHERE stripe_customer_id = ? LIMIT 1`
    )
      .bind(customerId)
      .first<LicenseRecord>();
  },
  ...
  async isEventProcessed(env: Env, stripeEventId: string): Promise<boolean> {
    const row = await env.DATABASE.prepare(
      `SELECT 1 FROM stripe_events WHERE stripe_event_id = ? AND status = 'success' LIMIT 1`
    )
      .bind(stripeEventId)
      .first();
    return row !== null;
