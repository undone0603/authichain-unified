...
test('Mini App defaults to the apex URL, never authichain.com/api/telegram', () => {
  ...
  assert.equal(miniAppUrl({ MINIAPP_URL: '  https://example.com/x  ' }), 'https://example.com/x')
...
test('webhook URL uses this worker origin, not SITE_URL/api/telegram', () => {
  const req = new Request('https://authichain-telegram.example.workers.dev/api/telegram/setup-webhook')
  assert.equal(
    webhookUrlFrom(req, {}),
    'https://authichain-telegram.example.workers.dev/api/telegram/webhook'
  )
  assert.equal(
    webhookUrlFrom(req, { TELEGRAM_WEBHOOK_URL: 'https://hooks.example/telegram' }),
