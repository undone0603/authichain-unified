...
const SERVICE_ENDPOINTS: {
  ...
  {
    name: "authichain-api",
    url: "https://authichain-api.undone-k.workers.dev/health",
  },
  {
    name: "qron-automation",
    url: "https://qron-automation.undone-k.workers.dev/health",
  },
  {
    name: "qron-outreach",
    url: "https://qron-outreach.undone-k.workers.dev/health",
  },
  {
    name: "resend-relay",
    url: "https://resend-relay.undone-k.workers.dev/health",
  },
  {
    name: "qron-portfolio",
    url: "https://qron-portfolio.undone-k.workers.dev/",
  },
  ...
  {
    name: "stripe-webhook (next.js)",
    url: "https://authichain.com/api/webhook",
    expectStatus: 405,
  },
];

async function checkEndpoint(
  endpoint: (typeof SERVICE_ENDPOINTS)[0]
): Promise<CheckResult> {
  const t0 = Date.now();
  try {
    ...
    const ok =
      ...
      (res.status >= 200 && res.status < 400 && !endpoint.expectStatus);
    return {
      name: endpoint.name,
      url: endpoint.url,
      status: ok ? "ok" : "warn",
  ...
  try {
    ...
    if (!res.ok)
      return {
        name: "stripe-webhook-config",
        status: "warn",
        detail: `Stripe API ${res.status}`,
      };
    const data: any = await res.json();
    const endpoints: any[] = data.data ?? [];
    const correctEndpoint = endpoints.find(
      (e: any) =>
        e.url === "https://authichain.com/api/webhook" && e.status === "enabled"
    );
    if (correctEndpoint) {
    ...
    const anyMatch = endpoints.find((e: any) => {
      try {
        return (h => h === "authichain.com" || h.endsWith(".authichain.com"))(
...
async function checkGitHubBranches(env: Env): Promise<CheckResult> {
  ...
  try {
    ...
    const stale = branches
      .filter((b: any) => !["main"].includes(b.name))
      .map((b: any) => b.name);
    return {
      name: "github-stale-branches",
  ...
  try {
    ...
    if (res.status === 403)
      return {
        name: "dependabot-alerts",
        status: "skip",
        detail: "no Dependabot access (enable in repo settings)",
      };
    if (!res.ok)
      return {
        name: "dependabot-alerts",
...
async function checkKVConnectivity(env: Env): Promise<CheckResult> {
  try {
    ...
    return {
      ...
      detail:
        val === probe ? "read/write ok" : "write succeeded but read mismatch",
    };
  } catch (e: any) {
...
async function runChecks(env: Env): Promise<InfraReport> {
  ...
  const serviceResults = await Promise.all(
    SERVICE_ENDPOINTS.map(checkEndpoint)
  );

  // Deeper checks
  const [stripeResult, kvResult, branchResult, dependabotResult] =
    await Promise.all([
  ...
  if (problems.length > 0) {
    ...
    if (prevRaw !== currentHash) {
      ...
      const body = [
        ...
        "--- Problems ---",
        ...problems.map(
          p => `[${p.status.toUpperCase()}] ${p.name}: ${p.detail ?? ""}`
        ),
        "",
        "--- All checks ---",
        ...checks.map(
          c =>
            `[${c.status}] ${c.name}${c.latencyMs ? ` (${c.latencyMs}ms)` : ""}: ${c.detail ?? ""}`
        ),
        "",
        `Run at: ${timestamp}`,
      ...
      if (emailRes.ok) {
        ...
        await env.INFRA_KV.put("last_alert_hash", currentHash, {
          expirationTtl: 86400,
        });
      }
    }
  ...
  await env.INFRA_KV.put("latest_report", JSON.stringify(report), {
  ...
  hist.unshift(report);
  hist.splice(24);
  await env.INFRA_KV.put("report_history", JSON.stringify(hist), {
...
export default {
  async fetch(request: Request, env: Env) {
    ...
    if (url.pathname === "/status") {
      ...
      if (!raw)
        return Response.json(
          { error: "No report yet — hit /run first" },
          { status: 404 }
        );
      const report: InfraReport = JSON.parse(raw);
      const statusEmoji = (s: string) =>
        s === "ok" ? "✅" : s === "warn" ? "⚠️" : s === "down" ? "🔴" : "⏭";
