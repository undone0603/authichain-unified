...
async function getOwnedProduct(productId: string, userId: number) {
...
export const supplyChainRouter = router({
  getEvents: protectedProcedure.input(z.object({ productId: z.string().uuid() })).query(async ({ ctx, input }) => {
