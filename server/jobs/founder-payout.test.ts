...
const { rev } = vi.hoisted(() => ({
  rev: { rows: [] as Array<{ amount: string }> },
}));
vi.mock("../db", () => ({
...
describe("runFounderPayout", () => {
  it("computes, logs, and notifies the owner", async () => {
    ...
    expect(logActivity).toHaveBeenCalledWith(
      expect.objectContaining({ action: "founder_payout_computed" })
    );
