...
vi.mock('./db-helpers', () => ({
  listPastDueSubscriptions: async (_db: unknown) => state.subs,
  hasDunningStepLogged: async (_db: unknown, id: number, step: string) => state.logged.has(`${id}:${step}`),
  createSystemNotification: (...a: unknown[]) => createSystemNotification(...a),
  logActivity: (...a: unknown[]) => logActivity(...a),
  getUserById: async (_db: unknown, id: number) => ({ id, email: `u${id}@example.com`, name: 'User' }),
}));
...
describe('runDunningEscalation', () => {
  it('sends a day_3 reminder for a 3–6 day past-due subscription', async () => {
    ...
    const r = await runDunningEscalation(fakeDb);
    expect(r.checked).toBe(1);
    expect(r.remindersSent).toBe(1);
    expect(logActivity).toHaveBeenCalledWith(fakeDb, expect.objectContaining({ action: 'billing_dunning_day_3' }));
  ...
  it('escalates to day_14 for a 14+ day past-due subscription', async () => {
    ...
    const r = await runDunningEscalation(fakeDb);
    expect(r.remindersSent).toBe(1);
    expect(logActivity).toHaveBeenCalledWith(fakeDb, expect.objectContaining({ action: 'billing_dunning_day_14' }));
  });
