...
const { state } = vi.hoisted(() => ({
  state: { subs: [] as Partial<Subscription>[], logged: new Set<string>() },
}));
...
vi.mock("../db", () => ({
  ...
  hasDunningStepLogged: async (id: number, step: string) =>
    state.logged.has(`${id}:${step}`),
...
describe("the dunning gate", () => {
  it('sends nothing unless DUNNING_ENABLED is exactly "true"', async () => {
    ...
    state.subs = [
      { id: 1, userId: 10, plan: "starter", updatedAt: daysAgo(20) },
    ];
    ...
    for (const value of [undefined, "false", "1", "TRUE", "yes"]) {
      ...
      expect(
        r.skipped,
        `DUNNING_ENABLED=${String(value)} should not send`
      ).toBe(true);
...
describe("runDunningEscalation", () => {
  it("sends a day_3 reminder for a 3–6 day past-due subscription", async () => {
    state.subs = [
      { id: 1, userId: 10, plan: "starter", updatedAt: daysAgo(4) },
    ];
    const r = await runDunningEscalation();
    expect(r.checked).toBe(1);
    expect(r.remindersSent).toBe(1);
    expect(logActivity).toHaveBeenCalledWith(
      expect.objectContaining({ action: "billing_dunning_day_3" })
    );
  ...
  it("escalates to day_14 for a 14+ day past-due subscription", async () => {
    state.subs = [
      { id: 2, userId: 11, plan: "professional", updatedAt: daysAgo(20) },
    ];
    const r = await runDunningEscalation();
    expect(r.remindersSent).toBe(1);
    expect(logActivity).toHaveBeenCalledWith(
      expect.objectContaining({ action: "billing_dunning_day_14" })
    );
  });

  it("is idempotent — skips a step that was already logged", async () => {
    state.subs = [
      { id: 3, userId: 12, plan: "starter", updatedAt: daysAgo(4) },
    ];
  ...
  it("does nothing for a subscription under 3 days past due", async () => {
    state.subs = [
      { id: 4, userId: 13, plan: "starter", updatedAt: daysAgo(1) },
    ];
    const r = await runDunningEscalation();
    expect(r.remindersSent).toBe(0);
