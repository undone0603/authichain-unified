...
import { getUserById, getDashboardMetrics, hasUserActionLogged, logActivity, type Db } from "../content-db-helpers";
 ...
 */
export async function checkUserMilestones(db: Db, userId: number) {
  ...
  try {
    ...
    if (metrics.totalAuthentications >= SCAN_THRESHOLD) {
      const alreadyLogged = await hasUserActionLogged(db, userId, "hubspot_milestone_scans");
      if (!alreadyLogged) {
        await createDeal({
        ...
        await logActivity(db, { userId, action: "hubspot_milestone_scans", details: { count: metrics.totalAuthentications } });
    ...
    if (agent && (agent.xp || 0) >= XP_THRESHOLD) {
      const alreadyLogged = await hasUserActionLogged(db, userId, "hubspot_milestone_reputation");
      if (!alreadyLogged) {
        await createDeal({
        ...
        await logActivity(db, { userId, action: "hubspot_milestone_reputation", details: { xp: agent.xp } });
