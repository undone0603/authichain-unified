...
export function getPriceId(planId: PlanId | string): string {
  const map: Record<string, string | undefined> = {
    authichain_basic:
      process.env.STRIPE_PRICE_AUTHICHAIN_BASIC ||
      process.env.STRIPE_PRICE_BASIC,
    authichain_pro:
      process.env.STRIPE_PRICE_AUTHICHAIN_PRO || process.env.STRIPE_PRICE_PRO,
    authichain_enterprise:
      process.env.STRIPE_PRICE_AUTHICHAIN_ENTERPRISE ||
  ...
  if (!priceId) {
    throw new Error(
      `No Stripe price ID configured for plan: ${planId}. Set STRIPE_PRICE_${planId.toUpperCase()} env var.`
    );
  }
  return priceId;
}
