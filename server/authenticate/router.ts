...
export const authenticateRouter = router({
  analyze: protectedProcedure.input(z.object({
    ...
    const quotaResult = await consumeSubscriptionQuota(db, ctx.user.id);
    ...
    const authResult = await createAuthentication(db, {
    ...
    if (aiResult.result === "authentic" && aiResult.confidence >= 80) {
      ...
      const cert = await createCertificate(db, {
    ...
    await recordUsage(db, { userId: ctx.user.id, subscriptionId: sub?.id, type: "authentication", quantity: 1 });
    await logActivity(db, { userId: ctx.user.id, action: "product_authenticated", entityType: "authentication", entityId: authResult.id });
    try {
  ...
  history: protectedProcedure.query(async ({ ctx }) => {
    const db = await getDb(); // see analyze() above
    return await getUserAuthentications(db, ctx.user.id);
  }),
});
