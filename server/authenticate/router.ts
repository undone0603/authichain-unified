...
export const authenticateRouter = router({
  analyze: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
        ...
        messages: [
          {
            role: "system",
            content:
              "You are an expert luxury product authenticator with blockchain verification capabilities. Analyze the provided product image and determine if it is authentic or counterfeit. Provide detailed reasoning, a confidence score (0-100), red flags, and authentic markers.",
          },
          {
            role: "user",
            content: [
              {
                type: "text" as const,
                text: `Authenticate this ${product.brand || ""} ${product.name}. Serial: ${product.serialNumber || "N/A"}. Category: ${product.category || "general"}`,
              },
              {
          ...
          json_schema: {
            ...
            schema: {
              ...
              properties: {
                result: {
                  type: "string",
                  enum: ["authentic", "counterfeit", "uncertain"],
                },
      ...
      await rewardAgentForVerification(
        ctx.user.id,
        aiResult.result === "authentic"
      );
      ...
      await db.recordUsage({
        ...
        type: "authentication",
        quantity: 1,
        billingPeriodStart: usagePeriodStart,
        billingPeriodEnd: usagePeriodEnd,
      });
      await db.logActivity({
        userId: ctx.user.id,
        action: "product_authenticated",
