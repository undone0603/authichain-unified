...
describe("assessRecipient (provenance guard)", () => {
  ...
  it("still rejects a published partner role inbox without allowRoleInbox", () => {
    const a = assessRecipient(
  ...
  it("allows a trusted role inbox only when allowRoleInbox is explicit", () => {
    const a = assessRecipient(
      "contact@existosolutions.com",
      "published_contact",
      {
    ...
    expect(a.isRoleInbox).toBe(true);
    expect(a.status).toBe("allow");
    expect(a.reasons).not.toContain("role_inbox");
    expect(canSend(a)).toBe(true);

    const ics = assessRecipient(
    ...
    expect(ics.status).toBe("allow");

    const inbound = assessRecipient("info@onnit.com", "inbound_optin", {
      allowRoleInbox: true,
    });
    expect(inbound.status).toBe("allow");
  });

  it("does not let allowRoleInbox waive an untrusted source", () => {
    const a = assessRecipient("info@guess.example", "pattern_guess", {
      allowRoleInbox: true,
    });
    expect(a.status).toBe("reject");
    expect(a.reasons).toContain("untrusted_source:pattern_guess");
  ...
  it("identifies role local-parts used by the partner list", () => {
    expect(isRoleInboxEmail("contact@existosolutions.com")).toBe(true);
    expect(isRoleInboxEmail("info@icsconsultingservice.com")).toBe(true);
    expect(isRoleInboxEmail("hello@pufcreativ.com")).toBe(true);
    expect(isRoleInboxEmail("fitzpatricks@nemcworks.org")).toBe(false);
    expect(isRoleInboxEmail("helpteam@oakleysign.com")).toBe(false);
  });

  it("rejects malformed addresses", () => {
    expect(assessRecipient("not-an-email", "confirmed_reply").status).toBe(
      "reject"
    );
  });
});
...
describe("guardedSend", () => {
  ...
  it("does not treat a role_inbox refuse as a live send attempt", async () => {
    const r = await guardedSend({
      to: "info@crescolabs.com",
      source: "inbound_optin",
      subject: "s",
    ...
    expect(countsAsLiveSendAttempt(r)).toBe(false);
  });
  ...
  it("counts a successful send and a Resend HTTP error against the live cap", () => {
    ...
    expect(
      countsAsLiveSendAttempt({ sent: true, id: "msg_1", assessment: base })
    ).toBe(true);
    expect(
      countsAsLiveSendAttempt({
        sent: false,
  ...
  it("sends a trusted partner role inbox when allowRoleInbox is set", async () => {
    ...
    const r = await guardedSend({
      to: "contact@existosolutions.com",
