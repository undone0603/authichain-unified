...
Self-Optimization: Loads "Win Signals" from the AuthiChain platform
to dynamically improve the outreach message tone and evidence.
"""
...
from agentz.core.intelligence import optimize_outreach_prompt
...
def run(ctx: ExecutionContext) -> str:
    # 1. Self-Optimize based on AuthiChain Platform Wins
    ctx.load_intelligence(limit=10)
    optimized_touch = optimize_outreach_prompt(FIRST_TOUCH, ctx.intelligence_signals or [])
    
    if ctx.intelligence_signals:
        ctx.step(f"self-optimized outreach message using {len(ctx.intelligence_signals)} platform signals")
    ...
    if ctx.mode == Mode.DRY_RUN:
        ...
        ctx.step(f"would send up to {remaining} connection requests with optimized message")
        return f"dry-run: would send up to {remaining}"

    return asyncio.run(_run_browser_task(ctx, remaining, optimized_touch))


async def _run_browser_task(ctx: ExecutionContext, budget: int, message_template: str) -> str:
    try:
