...
- **Not the cause:** `$0` amount, `is_demo`, `smoke_*` visit id, or missing `metadata.offer`. `fulfillDppPaidSession` has no amount/smoke filter. Live `smoke_check_1789786486` (`cs_live_a1y4Tu…`) is `payment_status=paid` `amount_total=0` with `offer=dpp_readiness_2026` + `plan=dpp_readiness`; funnel only has `checkout_started`.
