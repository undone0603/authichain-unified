...
Supabase project's _direct_ database hostname — which has **no IPv4 (A)
...
already have `if (!url || !anonKey)` guards for _missing_ env vars, so
...
_already_ manual-only by design — its own header comment says "Vercel's
...
| ------------------------ | ------------------------------------------------------------------------------------------------------------------------ | ---------------------------------------------------------------------------- |
| browser-vision-tasks.yml | Workflow-side fix applied; real fix is a secret change (DATABASE_URL → pooler host)                                      | Verified the _diagnosis_ live; job still fails until the secret is corrected |
