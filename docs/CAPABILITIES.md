...
- **x402 agent pay (LIVE, not `not_configured`):** `GET https://authichain.com/api/x402/health` → 200 `ready` / `trustless`. PayTo / tokenomics EOA `0x5db511706FB6317cd23A7655F67450c5AC6e6AA2` (not the NFT deployer), Base `8453`, Circle USDC `0x833589fCD6eDb6E08f4c7C32D4f71b54bdA02913`, **$0.05 / call**, PayAI facilitator reachable. `POST /api/x402` returns **402** with asset + EIP-712 extra when unpaid. Public docs: `https://authichain.com/x402`. Machine catalog: `GET /api/x402/catalog` and `/.well-known/x402.json` (prices copied from health). Tokenomics: `docs/strategy/AGENT_TOKENOMICS_x402.md`. Identity: `docs/strategy/WEB3_IDENTITY.md`. `$QRON` is not this rail. Do not rebind payTo / facilitator. Live self-pay smokes already succeeded — do not treat another settle as required.
