...
// Shared Stripe event handler — the single source of truth for fulfillment.
// Both /api/webhook and /api/stripe/webhook delegate here so that payment
// fulfillment does not depend on which URL is registered as the Stripe
// endpoint: whichever one Stripe actually calls, the same logic runs.
...
// --- Stripe helper (for follow-up API reads, e.g. subscription period end) ---
...
async function getStripeClient(): Promise<Stripe | null> {
  ...
  if (!_stripe) {
    ...
    // @ts-expect-error - apiVersion pinned string may lag the installed SDK's type union
    _stripe = new StripeCtor(key, { apiVersion: '2024-06-20' });
  }
  return _stripe;
}

// --- Metadata helpers (accept both camelCase and snake_case naming,
// since checkout sessions have historically been created with either) ---

function getPlanId(metadata: Stripe.Metadata | null | undefined): string | undefined {
  return metadata?.planId || metadata?.plan;
}

function getUserId(metadata: Stripe.Metadata | null | undefined): string | undefined {
...
//
// The dashboard/upsell/churn-risk/feature-flags/email-broadcast routes all
// read a user_id-keyed `subscriptions` row (columns: user_id, plan, status,
// current_period_end, stripe_customer_id, stripe_subscription_id) — but
// nothing was ever writing to it, so those routes always saw an empty
// subscription regardless of payment state. Keep it in sync here, non-fatal
// like every other side effect in this file: if the table/columns don't
// match what's actually deployed, log and move on rather than failing the
// webhook.
...
async function upsertSubscriptionRecord(params: {
  ...
  const { userId, planId, status, currentPeriodEnd, stripeCustomerId, stripeSubscriptionId } = params;
  if (!userId) return;
  try {
    const supabase = await getServiceClient();
    ...
    console.log(`[webhook] subscriptions row synced for user=${userId} status=${status}`);
  } catch (err) {
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[webhook] upsertSubscriptionRecord error (non-fatal):', err);
    await logAutomation('stripe_webhook.upsertSubscriptionRecord', 'event', 'failure', { userId, planId, status }, msg);
  }
}

async function cancelSubscriptionRecord(stripeCustomerId: string) {
  try {
    const supabase = await getServiceClient();
    await supabase
      .from('subscriptions')
    ...
    const msg = err instanceof Error ? err.message : String(err);
    console.error('[webhook] cancelSubscriptionRecord error (non-fatal):', err);
    await logAutomation('stripe_webhook.cancelSubscriptionRecord', 'event', 'failure', { stripeCustomerId }, msg);
  }
}

// --- Subscription welcome/confirmation email ---
//
// generateAndDeliverQr/fulfillStoryMode already email one-time (`mode:
// 'payment'`) purchasers. Recurring-plan (`mode: 'subscription'`) purchasers
// never got a confirmation email at all — this closes that gap.
...
async function sendSubscriptionWelcomeEmail(
  ...
  const planName = planId ? planId.replace(/_/g, ' ') : 'your plan';
  const result = await sendEmail({
    to,
    from: process.env.SENDGRID_FROM_EMAIL || 'QRON <hello@qron.space>',
    subject: 'Your QRON Subscription is Active',
    html: `
      <div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;background:#0a0a0a;color:#ededed;padding:40px 32px;border-radius:16px;border:1px solid rgba(201,162,39,0.2);">
        <h1 style="color:#c9a227;">You're All Set</h1>
        <p style="color:#9e9e9e;">Your <strong style="color:#c8c8c8;text-transform:capitalize;">${planName}</strong> subscription (${tier || 'pro'} tier) is now active.</p>
    ...
    text: `Your ${planName} subscription (${tier || 'pro'} tier) is now active. Manage it at ${dashUrl}`,
  });
  if (!result.ok) {
    console.warn('[webhook] subscription welcome email failed:', result.provider, result.error);
    await logAutomation('stripe_webhook.sendSubscriptionWelcomeEmail', 'event', 'failure', { to, planId, provider: result.provider }, result.error);
  }
}
...
export async function handleStripeWebhook(event: Stripe.Event): Promise<void> {
  ...
  switch (event.type) {
    case 'checkout.session.completed': {
      ...
      const planId = getPlanId(session.metadata);
      ...
      if (session.mode === 'payment') {
        ...
        const tier = planId ? PLAN_TIER[planId as PlanId] : undefined;
        ...
        const customerEmail =
          session.customer_email || session.customer_details?.email;
        if (customerEmail) {
          await sendSubscriptionWelcomeEmail(customerEmail, planId, tier);
        }
      }
      break;
    }
    ...
    case 'customer.subscription.updated': {
      ...
      const planId = getPlanId(sub.metadata);
    ...
    case 'customer.subscription.deleted': {
      ...
      if (customerId) {
        ...
        await cancelSubscriptionRecord(customerId);
      }
      break;
    }
