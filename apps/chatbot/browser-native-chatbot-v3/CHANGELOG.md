...
- Use `isLocal: true` in `connectOverCDP` (Playwright 1.58) for localhost CDP connections, enabling local file system optimisation for uploads and downloads.
- Persist IndexedDB data in session state save/restore via `storageState({ indexedDB: true })` (Playwright 1.51), improving fidelity of saved sessions.
- Add `partitionKey` field to `cookies_set` protocol schema (Playwright 1.54) to support partitioned cookies.
- Fix CDP error hint: the `--remote-debugging-port` hint now only appears when a bare port number was given, not when a full WebSocket URL pointing at localhost was provided.
- Fix two pre-existing TypeScript errors in `src/actions.ts` caused by DOM methods being called inside `evaluate()` callbacks without DOM lib types.
