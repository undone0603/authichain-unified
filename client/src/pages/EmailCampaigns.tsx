...
import { Mail, Loader2, Plus, Send, Sparkles, XCircle, Rocket, Zap, CheckCircle2 } from "lucide-react";
...
export default function EmailCampaigns() {
  ...
  const bulkApprove = trpc.emailDrafts.bulkApprove.useMutation({
    onSuccess: (res) => { toast.success(`Approved & sent ${res.count} draft(s)`); setSelected(new Set()); refetchDrafts(); },
    onError: (e) => toast.error(e.message),
  });
  ...
  const allDraftIds: number[] = (drafts ?? []).map((d: any) => d.id);
            ...
            <div className="grid gap-4">
              ...
              <div className="flex items-center justify-between gap-3 p-3 rounded-lg border bg-muted/20">
                <label className="flex items-center gap-2 text-sm cursor-pointer">
                  ...
                  {selected.size > 0 ? `${selected.size} selected` : `Select all (${allDraftIds.length})`}
                </label>
                <Button
                  size="sm"
                  className="gap-2"
                  disabled={selected.size === 0 || bulkApprove.isPending}
                  onClick={() => bulkApprove.mutate({ ids: Array.from(selected) })}
                >
                  {bulkApprove.isPending ? <Loader2 className="h-3.5 w-3.5 animate-spin" /> : <CheckCircle2 className="h-3.5 w-3.5" />}
                  Approve &amp; Send {selected.size > 0 ? `(${selected.size})` : ""}
                </Button>
              </div>
              {drafts.map((d: any) => (
                <Card key={d.id} className={`overflow-hidden ${selected.has(d.id) ? "ring-2 ring-primary/40" : ""}`}>
                  <CardHeader className="bg-muted/30 pb-3">
                    <div className="flex items-center justify-between">
                      <div className="flex items-center gap-3">
                        <Checkbox checked={selected.has(d.id)} onCheckedChange={() => toggleSelected(d.id)} />
                        <div>
                      ...
                      <Badge variant="outline" className="bg-background">{d.generatedBy ? d.generatedBy.replace(/_/g, " ") : "AgentZ"}</Badge>
                    </div>
                  </CardHeader>
