...
async function lockQronEscrow(
  ...
  const contractAddress = env.POLYGON_QRON_TOKEN;
  ...
  console.log(`[qron-bridge] Locking ${qronAmount} QRON (${amountUsd} USD) for ref=${contractRef} at address=${contractAddress} using RPC=${env.POLYGON_RPC_URL}`);
