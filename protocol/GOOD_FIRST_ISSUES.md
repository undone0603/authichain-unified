...
## 4. Fix the relative license link in `SPEC.md`
`SPEC.md` §0 points to `../LICENSE.md`. That file does not exist at that path (the platform license is `docs/project/LICENSE.md`), and relative links break when the spec is read outside the repo.
- **Do:** point it to the correct file with an absolute GitHub URL, and check the other links in `SPEC.md` and `README.md`.
- **Done when:** every link in `protocol/*.md` resolves.

## 5. Show example output for each verdict in `README.md`
...
## 6. (stretch, "help wanted") A second-language verifier
