...
def run(ctx: ExecutionContext) -> str:
    ...
    try:
        ...
        # 1. Fetch & Prioritize Hot Leads
        if ctx.mode == Mode.DRY_RUN:
            leads = [
                {"id": "mock_1", "name": "Cloud Cannabis", "slug": "cloud-cannabis", "sentiment_score": 9.0},
                {"id": "mock_2", "name": "Oakley Signs", "slug": "oakley-signs", "sentiment_score": 8.5}
            ]
            ...
            # Prioritize and filter by sentiment
            prioritized = asyncio.run(prioritize_leads_by_sentiment(leads_res)) if leads_res else []
            leads = [l for l in prioritized if l.get("sentiment_score", 0) > 7.0]

        if not leads:
            return "No high-intent leads identified in HubSpot."

        ctx.step(f"Identified {len(leads)} high-intent, high-priority leads.")
        ...
        for lead in leads:
            try:
                ...
                if ctx.mode == Mode.AUTO:
                    # Use fallback research in auto mode to avoid browser-use deadlocks
                    research = {
                        "decision_maker_name": "Decision Maker",
                        "latest_news": "your recent market expansion"
                    }
                    ctx.step(f"Using fallback research for {name} to avoid browser deadlock.")
                elif ctx.mode != Mode.DRY_RUN:
