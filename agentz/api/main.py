...
async def api_redeem(wallet: str, amount: float, business_id: str, supabase: Client = Depends(get_supabase)):
    """Operator-only. Records a redemption without checking a $QRON balance
    or burning anything on-chain; the response says `simulated: true`. Needs a
    real balance check and burn before it is opened to users."""
...
async def get_operational_results():
    """Lists the report and agreement files AgentZ has written.

    Counts are derived from what is on disk. This used to also return a
    hard-coded deal count and ARR figure, which no data backed.
    """
    import glob
