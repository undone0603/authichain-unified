...
export const BRAND_IDS = ['authichain', 'qron', 'strainchain', 'govchain'];
export const DEFAULT_BRAND = 'authichain';
/**
...
export function resolveBrand(hostname) {
    ...
    for (const brand of Object.values(BRANDS)) {
        if (host === brand.domain)
            return brand.id;
        if (brand.aliases.includes(host))
            return brand.id;
    }
    // Pass 2: subdomain of a known domain
    for (const brand of Object.values(BRANDS)) {
        if (host.endsWith('.' + brand.domain))
            return brand.id;
        for (const alias of brand.aliases) {
            if (host.endsWith('.' + alias))
                return brand.id;
        }
    }
    // Pass 3: brand id anywhere in the hostname (preview URLs)
    for (const brand of Object.values(BRANDS)) {
        if (host.includes(brand.id))
            return brand.id;
    }
    return DEFAULT_BRAND;
}
