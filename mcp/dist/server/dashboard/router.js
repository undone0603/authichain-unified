    ...
    pulse: publicProcedure.query(async () => {
        ...
        return activity.map(a => {
            ...
            if (a.action === "strainchain_auto_anchor") {
                text = `Bitcoin L1 Anchor: Package #${a.details?.packageTag?.substring(0, 8)}...`;
            }
            else if (a.action === "roi_calculated") {
                text = `MedTech ROI Analysis: $${(a.details?.savings || 250000).toLocaleString()} Savings...`;
            }
            else if (a.action === "govchain_passport_issued") {
                text = `Sovereign Passport Issued: #${a.details?.documentId}...`;
            }
            else if (a.action === "nft_minted") {
                text = `Authenticity Token Minted: ${a.details?.name}...`;
            }
            else if (a.action === "product_authenticated") {
                text = `High-Fidelity Scan Verified: Confidence ${a.details?.confidence}%...`;
            }
            else if (a.action === "qron_staked") {
                text = `Validator Stake Active: ${a.details?.amount} $QRON Locked...`;
            }
