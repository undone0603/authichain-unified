...
import { registerGmailOAuthRoutes } from "../../.github-staging/gmail-oauth";
...
import { oauthRateLimit, contactRateLimit, gptRateLimit, globalApiRateLimit, adminRateLimit, } from "./rate-limit";
/**
...
export function createApp() {
    ...
    app.post("/api/webhooks/docusign", async (req, res) => {
        ...
        if (secret) {
            const provided = req.headers["x-docusign-secret"];
            if (!provided || !timingSafeEqual(provided, secret)) {
                return res.status(401).json({ error: "Invalid webhook secret" });
            }
        }
    ...
    if (!process.env.PADDLE_WEBHOOK_SECRET) {
        console.warn("[Paddle] PADDLE_WEBHOOK_SECRET is not set. " +
