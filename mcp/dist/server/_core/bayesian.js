...
function _sampleGamma(shape) {
    if (shape < 1)
        return _gammaGT(1 + shape) * Math.pow(Math.random(), 1 / shape);
...
export function updatePrior(prior, outcome) {
...
export function thompsonSample(arms) {
    let best = arms[0];
...
export function selectTone(segment) {
 ...
 */
export function ucb1Score(prior, totalTasks) {
    const n = prior.alpha + prior.beta - 2; // actual observations
    if (n <= 0)
        return Infinity; // unexplored → highest priority
...
export function rankMissions(candidates) {
