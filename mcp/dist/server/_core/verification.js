...
export function createVerificationRecord(payload, ttlMs = DEFAULT_TTL_MS) {
...
export function verifyHash(incomingHash, record) {
    const now = Date.now();
    if (record.revoked) {
        return {
...
export function buildVerificationUrl(baseUrl, productId, hash) {
    const url = new URL('/verify', baseUrl);
    url.searchParams.set('id', productId);
    url.searchParams.set('hash', hash);
    return url.toString();
}
export function issueVerificationUrl(baseUrl, productId, batchId) {
    const payload = { productId, batchId, timestamp: Date.now() };
 ...
 */
export function quickVerify(productId, hash, toleranceMs = 60_000) {
