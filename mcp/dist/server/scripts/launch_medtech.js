...
async function launchMedTech() {
    ...
    try {
        ...
        const missionId = await createMission("MEDTECH_OUTREACH");
        console.log(`✅ Mission Created: ${missionId}`);
        // 2. Enqueue the sequence tasks
        await enqueueTask(missionId, "FIND_MEDTECH_LEADS", { count: 10, icp: "Compliance Director at medical device manufacturer" });
        await enqueueTask(missionId, "DRAFT_OUTBOUND_EMAIL", { segment: "MEDTECH", sequence: 1 });
