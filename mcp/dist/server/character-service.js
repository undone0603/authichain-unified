...
import { characterGenerations, characterAssets, protocolAgents, verificationClaims, consensusResults, qronRewardLedger, checkpointBatches, } from "../drizzle/schema";
...
export async function startCharacterGeneration(userId, archetype, context) {
...
Return ONLY a JSON object with these exact keys and float scores (e.g., 7.5).`,
        ...
        const scores = parseLLMContent(content);
        // Calculate weighted total (out of 10)
        const totalScore = (scores.protocol_fit * 0.20 +
...
export async function selectCharacterAsset(userId, assetId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    // Verify ownership
    ...
    const { url: metadataUri } = await storagePut(`character-metadata/${assetId}-${metadataHash.slice(0, 8)}.json`, Buffer.from(metadataJson), "application/json");
    // Mark selected
    await db.update(characterAssets)
        .set({
        isSelected: 1,
...
export async function prepareMint(userId, assetId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    ...
    const [asset] = await db.select()
        .from(characterAssets)
        .innerJoin(characterGenerations, eq(characterAssets.generationId, characterGenerations.id))
        .where(and(eq(characterAssets.id, assetId), eq(characterGenerations.userId, userId), eq(characterAssets.isSelected, 1)))
        .limit(1);
    if (!asset)
        throw new Error("Asset not found, not owned, or not selected");
...
// ─── Agent Creation ─────────────────────────────────────────────────────────
export async function createProtocolAgent(userId, characterAssetId, name, agentType) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
...
export async function getUserCharacterAssets(userId) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select({
...
// ─── QRON Rewards ───────────────────────────────────────────────────────────
export async function awardQRON(agentId, userId, amount, reason, referenceType, referenceId) {
    const db = await getDb();
    if (!db)
        return;
...
export async function getAgentRewards(agentId, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
    return db.select()
...
export async function getNetworkStats() {
    ...
    const [qronSum] = await db.select({
        total: sql `COALESCE(SUM(${qronRewardLedger.amount}), 0)`,
    }).from(qronRewardLedger);
...
export async function submitVerificationClaim(agentId, productId, authenticationId, claimType, confidence, evidence, reasoning) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    const [agent] = await db.select()
 ...
 */
export async function rewardAgentForVerification(userId, wasSuccessful) {
    const db = await getDb();
    if (!db)
        return;
