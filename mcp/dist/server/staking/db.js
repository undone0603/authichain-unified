...
import { stakingPositions, platformFees, transactions } from "../../drizzle/schema";
/**
 ...
 */
export async function getActiveStakingPositions(userId) {
    const db = await getDb();
    if (!db)
        return [];
    return await db
 ...
 */
export async function calculateRewards(positionId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    // Get position
 ...
 */
export async function withdrawStaking(positionId, userId) {
    const db = await getDb();
    if (!db)
        throw new Error("Database not available");
    // Get position
 ...
 */
export async function getUserStakingStats(userId) {
    const db = await getDb();
    if (!db) {
        return {
            totalStaked: 0,
