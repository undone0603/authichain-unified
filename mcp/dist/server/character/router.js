...
export const characterRouter = router({
    ...
    createAgent: protectedProcedure.input(z.object({
        ...
        return await service.createProtocolAgent(ctx.user.id, input.characterAssetId, input.agentName || input.name || "", input.agentType);
    }),
    ...
    submitClaim: protectedProcedure.input(z.object({
        ...
        if (!myAgent || myAgent.id !== input.agentId)
            throw new TRPCError({ code: "FORBIDDEN", message: "Agent not owned by user" });
        return await service.submitVerificationClaim(input.agentId, input.productId, input.authenticationId, input.claimType, input.confidence, input.evidence, input.reasoning);
    }),
});
