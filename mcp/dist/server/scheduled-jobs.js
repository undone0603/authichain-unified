...
import { scheduledJobRuns, subscriptions, certificates, leads, notifications, users, authentications, payments, customerHealthScores, fraudAlerts, stakingPositions, qronRewardLedger } from "../drizzle/schema";
...
async function isJobDue(db, job) {
    if (job.minIntervalMs <= 0)
        return true;
...
registerJob({
    ...
    handler: async () => {
        const db = await getDb();
        if (!db)
            return { itemsProcessed: 0, details: { error: "No DB" } };
        const now = new Date();
        ...
        const expiringSubs = await db.select()
            .from(subscriptions)
            .where(and(eq(subscriptions.status, "active"), lte(subscriptions.currentPeriodEnd, threeDaysFromNow), gte(subscriptions.currentPeriodEnd, now)))
            .limit(1000);
...
registerJob({
    ...
    handler: async () => {
        ...
        const expiringCerts = await db.select()
            .from(certificates)
            .where(and(eq(certificates.status, "active"), lte(certificates.expiresAt, thirtyDaysFromNow), gte(certificates.expiresAt, now)))
            .limit(1000);
        ...
        await db.update(certificates)
            .set({ status: "expired" })
            .where(and(eq(certificates.status, "active"), lt(certificates.expiresAt, now)));
        return {
...
registerJob({
    ...
    handler: async () => {
        ...
        await db.delete(notifications)
            .where(and(eq(notifications.isRead, 1), lt(notifications.createdAt, thirtyDaysAgo)));
        details.oldNotificationsDeleted = "checked";
        processed++;
        // Delete completed job runs older than 90 days
        await db.delete(scheduledJobRuns)
            .where(and(eq(scheduledJobRuns.status, "completed"), lt(scheduledJobRuns.startedAt, ninetyDaysAgo)));
...
registerJob({
    ...
    handler: async () => {
        ...
        for (const sub of activeSubs) {
            ...
            let trend = "stable";
            if (existing) {
                if (score > existing.score + 5)
                    trend = "improving";
                else if (score < existing.score - 5)
                    trend = "declining";
            }
...
registerJob({
    ...
    handler: async () => {
        ...
        const failedAuths = await db.select({
            ...
            .from(authentications)
            .where(and(gte(authentications.createdAt, sixHoursAgo), eq(authentications.result, "counterfeit")))
...
export async function initializeScheduler() {
    ...
    for (const job of jobs) {
        ...
        const task = cron.schedule(job.schedule, () => {
            executeJob(job).catch(err => console.error(`[Scheduler] Unhandled error in ${job.name}:`, err));
        });
...
export async function getJobHistory(jobName, limit = 50) {
    const db = await getDb();
    if (!db)
        return [];
    if (jobName) {
...
export async function runJobManually(jobName, options) {
