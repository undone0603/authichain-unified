...
const ACTIONS = {
    draftSalesEmail: {
        ...
        run: (input) => agent.draftSalesEmail(input),
    },
    draftPartnershipEmail: {
        ...
        run: (input) => agent.draftPartnershipEmail(input),
    },
    generateLinkedInPost: {
        label: "LinkedIn Post",
        description: "Full LinkedIn post on a topic.",
        example: { topic: "Michigan cannabis tax compliance" },
        run: (input) => agent.generateLinkedInPost(input.topic, input.options),
    },
    generateBlogPost: {
        label: "Blog Post",
        ...
        run: (input) => agent.generateBlogPost(input.topic, input.length, input.keywords),
    },
    generateProductAnnouncement: {
        ...
        run: (input) => agent.generateProductAnnouncement(input),
    },
    generateSocialMediaContent: {
        ...
        run: (input) => agent.generateSocialMediaContent(input.platform, input.topic, input.style),
    },
    analyzeCompetitor: {
    ...
    improveWriting: {
        ...
        run: (input) => agent.improveWriting(input.text, input.style, input.purpose),
    },
