 ...
 */
export async function generateProductQRON(params) {
    ...
    if (!imageUrl)
        throw new Error("QRON API did not return an image URL");
...
export function computeTrustScore(params) {
    ...
    const score = ((layers.qrDecode.pass ? layers.qrDecode.weight : 0) +
...
async function registerToOpenART(params) {
    ...
    if (!res.ok)
        throw new Error(`openART register failed: ${res.status}`);
    const json = await res.json();
