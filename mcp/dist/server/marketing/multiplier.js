 ...
 */
export async function runSocialMultiplier(announcement) {
...
Return JSON:
{
  ...
  `;
    try {
        ...
        return parseLLMContent(result.choices[0].message.content);
    }
    catch (err) {
        ...
        try {
            ...
            if (m)
                announcedHost = new URL(m[0]).hostname.toLowerCase().replace(/^www\./, "");
        }
        catch { /* no valid URL in text */ }
        const isHost = (d) => announcedHost === d || announcedHost.endsWith(`.${d}`);
