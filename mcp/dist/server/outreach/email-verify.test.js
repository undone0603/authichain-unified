...
function mockReacher(byEmail) {
    vi.spyOn(globalThis, 'fetch').mockImplementation(async (_url, init) => {
        const { to_email } = JSON.parse(init.body);
        const r = byEmail[to_email] ?? { is_reachable: 'invalid' };
        return { ok: true, json: async () => ({ is_reachable: r.is_reachable, smtp: { is_catch_all: r.is_catch_all ?? false } }) };
    });
}
