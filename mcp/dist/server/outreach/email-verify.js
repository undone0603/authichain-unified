...
const reacherUrl = () => process.env.REACHER_URL ?? "http://localhost:8080";
const clean = (s) => (s || "").trim().toLowerCase().replace(/[^a-z0-9]/g, "");
const cleanDomain = (domain) => (domain || "")
    .trim()
    .toLowerCase()
    .replace(/^https?:\/\//, "")
    .replace(/^@/, "")
...
export function inferEmailCandidates(firstName, lastName, domain) {
