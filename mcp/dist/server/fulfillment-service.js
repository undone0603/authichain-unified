 ...
 */
export async function processPhysicalFulfillment(request) {
    ...
    try {
        ...
        if (mockResponse.success) {
            ...
            if (order?.userId) {
                await db.createSystemNotification(order.userId, "Security Seals Dispatched 📦", `Your 1,000 scannable security seals for Order #${request.orderId} are on the way. Tracking: ${mockResponse.trackingNumber}`, "alert", "/services");
            }
 ...
 */
export async function triggerFulfillmentFromPayment(sessionId) {
    ...
    if (process.env.FULFILLMENT_MOCK_ENABLED !== "true") {
        console.log(`[Fulfillment] Skipped for order #${order.id} (${order.serviceType}) — ` +
