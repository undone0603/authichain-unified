 ...
 */
export async function generatePersonalizedContent(context, baseContent) {
 ...
 */
export async function generatePersonalizationRules(targetElement, baseContent) {
 ...
 */
export function detectSegment(context) {
    const segments = [];
    // Geographic segments
    ...
    if (context.trafficSource?.includes("google"))
        segments.push("search");
    else if (context.trafficSource?.includes("linkedin"))
        segments.push("linkedin");
    else if (context.trafficSource?.includes("facebook") || context.trafficSource?.includes("twitter"))
    ...
    // Campaign segments
    if (context.utmCampaign?.includes("brand"))
        segments.push("brand_aware");
 ...
 */
export function matchRules(context, rules) {
 ...
 */
export function detectTrafficSource(referrer) {
    if (!referrer)
        return "direct";
    const lowerReferrer = referrer.toLowerCase();
    if (lowerReferrer.includes("google"))
        return "google";
    if (lowerReferrer.includes("bing"))
        return "bing";
    ...
    if (lowerReferrer.includes("twitter") || lowerReferrer.includes("t.co"))
        return "twitter";
    if (lowerReferrer.includes("reddit"))
        return "reddit";
    return "referral";
}
/**
 ...
 */
export async function analyzePersonalizationPerformance(rules) {
