...
export const subscriptionsRouter = router({
    ...
    create: adminProcedure.input(z.object({
        ...
        const result = await db.createSubscription({
            userId: input.userId, plan: input.plan, monthlyQuota: quotas[input.plan] ?? 0,
    ...
    usage: protectedProcedure.query(async ({ ctx }) => {
        const sub = await db.getUserSubscription(ctx.user.id);
        if (!sub)
            return { plan: null, used: 0, limit: 0, percentage: 0 };
    ...
    checkout: protectedProcedure.input(z.object({
        ...
        const url = await stripeService.createSubscriptionCheckout({
            ...
            origin: input.origin,
            stripeCustomerId: ctx.user.stripeCustomerId || undefined,
            trialDays,
        });
    ...
    createPaddleCheckout: protectedProcedure.input(z.object({
        ...
        if (!priceId)
            throw new TRPCError({ code: "BAD_REQUEST", message: `Paddle price not configured for ${input.plan}/${input.billing}` });
