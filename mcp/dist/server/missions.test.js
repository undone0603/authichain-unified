...
vi.mock('./missions/missions.db', () => ({
    getMissions: async (statusFilter) => {
    ...
    getMissionById: async (id) => {
        ...
        if (!m)
            return null;
        const tasks = [...store.tasks.values()].filter((t) => t.missionId === id);
        return { ...m, tasks };
    },
    createMission: async (type) => {
    ...
    updateMissionStatus: async (id, status) => {
        const m = store.missions.get(id);
        if (m)
            store.missions.set(id, { ...m, status, updatedAt: new Date() });
    },
    getTasksByMission: async (missionId) => {
        return [...store.tasks.values()].filter((t) => t.missionId === missionId);
    },
    retryTask: async (id) => {
        const t = store.tasks.get(id);
        if (t)
            store.tasks.set(id, { ...t, status: 'PENDING', lastError: null, retryCount: 0, retryAfter: null, updatedAt: new Date() });
    },
}));
...
describe('missions — auth guards', () => {
    ...
    it('tasks.list throws UNAUTHORIZED for unauthenticated', async () => {
        const caller = appRouter.createCaller(publicCtx());
        await expect(caller.tasks.list({ missionId: '00000000-0000-0000-0000-000000000000' })).rejects.toThrow();
    });
});
// ─── missions.list ────────────────────────────────────────────────────────────
describe('missions.list', () => {
    ...
    it('filters by status', async () => {
        ...
        expect(Array.isArray(result)).toBe(true);
        result.forEach((m) => expect(m.status).toBe('PLANNED'));
    });
});
...
describe('missions.create', () => {
    ...
    it('missions.get returns mission with correct type', async () => {
        const caller = appRouter.createCaller(makeCtx('admin'));
        const result = await caller.missions.get({ id: missionId });
    ...
    it('tasks.list returns same tasks for mission', async () => {
        const caller = appRouter.createCaller(makeCtx('admin'));
        const fromGet = (await caller.missions.get({ id: missionId }))?.tasks ?? [];
...
describe('missions.create — task counts', () => {
    ...
    for (const [type, count] of Object.entries(expectedTaskCounts)) {
        it(`${type} creates ${count} tasks`, async () => {
            const caller = appRouter.createCaller(makeCtx('admin'));
            const { id } = await caller.missions.create({ type: type });
...
describe('missions.updateStatus', () => {
    ...
    it('transitions PLANNED → IN_PROGRESS', async () => {
        ...
        const mission = await caller.missions.get({ id: missionId });
...
describe('tasks.retry', () => {
    ...
    beforeAll(async () => {
        ...
        if (t)
            store.tasks.set(failedTaskId, { ...t, status: 'FAILED', lastError: 'test failure', updatedAt: new Date() });
...
describe('missions.list — status filtering after mutations', () => {
    it('lists IN_PROGRESS missions after status update', async () => {
        ...
        const inProgress = await caller.missions.list({ status: 'IN_PROGRESS' });
        expect(inProgress.some((m) => m.id === id)).toBe(true);
        const planned = await caller.missions.list({ status: 'PLANNED' });
        expect(planned.every((m) => m.id !== id)).toBe(true);
    });
});
