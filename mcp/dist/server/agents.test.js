...
// ─── Helpers ──────────────────────────────────────────────────────────────────
function makeTask(kind, payload = {}) {
    return {
        id: 'task-test-001',
...
function llmContentResponse(content) {
    return {
        id: 'mock',
        created: 0,
        model: 'mock',
...
describe('runLeadFinder', () => {
    ...
    it('enqueues a BROWSE_RESEARCH_LEAD task for each valid lead', async () => {
        ...
        expect(enqueueTask).toHaveBeenCalledWith('mission-test-001', 'BROWSE_RESEARCH_LEAD', expect.objectContaining({ leadEmail: 'alice@gov.com', segment: 'GOV' }));
    });
    ...
    it('uses RETAIL segment for FIND_RETAIL_LEADS task kind', async () => {
        ...
        const prompt = invokeLLM.mock.calls[0][0].messages[0].content;
    ...
    it('logs activity on completion', async () => {
        ...
        const { runLeadFinder } = await import('./agents/lead-finder.js');
        await runLeadFinder(makeTask('FIND_GOV_LEADS', { segment: 'GOV' }));
        expect(logActivity).toHaveBeenCalledWith(expect.objectContaining({ action: 'lead_finder_completed', details: expect.objectContaining({ source: 'apollo' }) }));
    });
...
describe('runOutboundEmail', () => {
    ...
    it('logs activity after sending', async () => {
        ...
        expect(logActivity).toHaveBeenCalledWith(expect.objectContaining({ action: 'outbound_email_sent' }));
    });
});
// ─── followup-sequence ────────────────────────────────────────────────────────
describe('runFollowupSequence', () => {
    ...
    it('logs and returns early when no DB connection', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'followup_skipped_no_db' }));
    });
});
// ─── pilot-packet ─────────────────────────────────────────────────────────────
describe('runBuildPilotPacket', () => {
    ...
    it('calls LLM and logs activity', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'pilot_packet_built' }));
    });
...
describe('runDraftIntelDossier', () => {
    ...
    it('calls LLM and logs activity', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'intel_dossier_drafted' }));
    });
});
// ─── crm-update ───────────────────────────────────────────────────────────────
describe('runCrmUpdate', () => {
    ...
    it('logs skip and returns when HubSpot is not configured', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'crm_update_skipped' }));
    ...
    it('syncs single lead when HubSpot is configured and leadEmail in payload', async () => {
        ...
        expect(vi.mocked(syncLeadToHubSpot)).toHaveBeenCalledWith(expect.objectContaining({ email: 'alice@gov.com' }));
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'crm_lead_synced' }));
    });
});
// ─── retail ───────────────────────────────────────────────────────────────────
describe('runFinalizeRetailSignage', () => {
    ...
    it('calls LLM and logs retail_signage_finalized', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'retail_signage_finalized' }));
    });
...
describe('runPackageSkuOnboarding', () => {
    ...
    it('calls LLM and logs sku_onboarding_packaged', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'sku_onboarding_packaged' }));
    });
});
// ─── content ──────────────────────────────────────────────────────────────────
describe('content agents', () => {
    ...
    it('runGenerateLaunchChecklist logs launch_checklist_generated', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'launch_checklist_generated' }));
    });
    it('runDraftLaunchEmail logs launch_email_drafted', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'launch_email_drafted' }));
    });
    it('runDraftPressRelease logs press_release_drafted', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'press_release_drafted' }));
    });
    it('runScheduleSocialPosts logs social_posts_scheduled', async () => {
        ...
        expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'social_posts_scheduled' }));
    });
...
describe('infra agents', () => {
    ...
    describe('runCheckDnsConfig', () => {
        it('completes without throwing when all DNS records resolve', async () => {
            ...
            expect(vi.mocked(logActivity)).toHaveBeenCalledWith(expect.objectContaining({ action: 'dns_config_checked' }));
        });
    ...
    describe('runLighthouseAudit', () => {
        it('logs scores and completes when PageSpeed API responds', async () => {
            ...
            expect(call.details.scores.performance).toBe(95);
            expect(call.details.scores.accessibility).toBe(88);
        });
