...
async function scoreleads(apolloLeads, segment, icp, conversionMean, ciLo, ciHi, expectedValue) {
    if (apolloLeads.length === 0)
        return [];
    const leadList = apolloLeads.map((l, i) => `${i}: name="${l.name}" title="${l.title}" org="${l.org}" industry="${l.orgIndustry ?? ''}" seniority="${l.seniority ?? ''}"`).join('\n');
...
export async function runLeadFinder(task) {
    ...
    const icp = payload.icp ?? (segment === 'GOV' ? 'government agency procurement and supply chain officer' :
