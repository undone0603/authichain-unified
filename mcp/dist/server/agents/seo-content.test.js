...
const { llm } = vi.hoisted(() => ({ llm: { content: '', throwOnce: false } }));
vi.mock('../_core/llm.js', () => ({
    ...
    parseLLMContent: (raw) => JSON.parse(raw),
}));
const logActivity = vi.fn(async (..._args) => { });
vi.mock('../db.js', () => ({ logActivity: (...a) => logActivity(...a) }));
...
describe('runProgrammaticSeoBatch', () => {
    it('generates pages for valid brand keys and logs the run', async () => {
        ...
        expect(logActivity).toHaveBeenCalledWith(expect.objectContaining({ action: 'programmatic_seo_generated' }));
    });
