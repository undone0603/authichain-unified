...
export async function runFollowupSequence(task) {
    ...
    const now = new Date();
    const dueLeads = await db.select().from(leads).where(and(eq(leads.segment, segment), inArray(leads.status, ['CONTACTED']), lte(leads.nextActionAt, now)));
    let drafted = 0;
    let sent = 0;
