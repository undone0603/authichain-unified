...
export async function runGenerateLaunchChecklist(task) {
...
Return JSON: { "subject": "...", "body": "..." }`;
    ...
    const email = parseLLMContent(result.choices[0].message.content);
...
export async function runDraftPressRelease(task) {
...
Return JSON: { "platforms": { "<platform>": [{ "day": 0, "copy": "...", "hashtags": ["..."] }] } }`;
    ...
    const calendar = parseLLMContent(result.choices[0].message.content);
    const postedUrls = [];
    ...
    const [twitterResults, linkedinResults] = await Promise.allSettled([
        ...
        (async () => {
            if (!(platforms.includes('twitter') || platforms.includes('x')) || twitterPosts.length === 0)
                return [];
    ...
    if (twitterResults.status === 'fulfilled')
        ...
        console.warn('[content.ts] Twitter post failed:', twitterResults.reason);
