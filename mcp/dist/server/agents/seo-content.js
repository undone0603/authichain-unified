...
export const BRAND_SEO = {
    authichain: {
        ...
        facts: 'AI + blockchain product authentication. 5-agent AI consensus, Bitcoin L1 anchoring, ' +
    ...
    strainchain: {
        ...
        facts: 'Consumer-facing blockchain provenance for cannabis. Integrates with METRC and BioTrack, ' +
    ...
    govchain: {
        brand: 'GovChain',
        domain: 'govchain.us',
        facts: 'On-chain government document verification. Bitcoin-anchored hashes for permits, ' +
    ...
    qron: {
        brand: 'QRON',
        domain: 'qron.space',
        facts: 'AI "living QR art" generator. ControlNet-generated scannable art QR codes, Ed25519-signed ' +
...
const clamp = (s, max) => s.length > max ? s.slice(0, max - 1).trimEnd() + '…' : s;
/**
 ...
 */
export async function generateSeoPage(config, keyword) {
...
Return JSON:
{
  ...
  "bodyHtml": "200-350 words of clean semantic HTML using <h2>/<p>/<ul> only — no <script>, no inline styles"
    ...
    const raw = parseLLMContent(result.choices[0].message.content);
 ...
 */
export async function runProgrammaticSeoBatch(jobs) {
    const pages = [];
