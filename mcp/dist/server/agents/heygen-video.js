...
import { listAvatars, listVoices, generateVideo, draftOutreachScript, getVideoStatus, } from '../heygen-service.js';
import { uploadVideo } from '../youtube-service.js';
const POLL_INTERVAL_MS = 8_000;
const MAX_POLLS = 30; // 4 minutes max wait
export async function runGenerateOutreachVideo(task) {
    ...
    if (!avatarId) {
        ...
        if (!avatars.length)
            throw new Error("No HeyGen avatars available");
    ...
    if (!voiceId) {
        ...
        if (!enVoice)
            throw new Error("No HeyGen voices available");
