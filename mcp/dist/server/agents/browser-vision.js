...
// ── Core vision loop ─────────────────────────────────────────────────────────
async function visionLoop(page, objective) {
    const findings = [];
    const history = [
        {
            role: 'user',
            ...
            switch (call.function.name) {
                ...
                case 'extract':
                    if (args.findings?.trim())
                        findings.push(args.findings.trim());
 ...
 */
export async function runVisionResearchLead(task) {
...
Extract a concise CRM note AND a one-sentence personalised email opener.`);
    ...
    const { crmNote, hookSentence } = parseLLMContent(summaryResult.choices[0].message.content);
 ...
 */
export async function runVisionFreeform(task) {
    const p = task.payload;
    const browser = await launchBrowser();
    const page = await browser.newPage();
    await page.setViewportSize(VIEWPORT);
