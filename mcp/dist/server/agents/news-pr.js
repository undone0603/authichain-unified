...
export async function runNewsjackingMonitor(task) {
...
Return JSON:
{
  ...
  `;
    try {
        ...
        const story = parseLLMContent(result.choices[0].message.content);
...
Return JSON: { "prTitle": "...", "prBody": "..." }
    `;
        ...
        const pr = parseLLMContent(prResult.choices[0].message.content);
