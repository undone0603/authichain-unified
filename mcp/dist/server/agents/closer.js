...
Return JSON: { "intent": "<one of: interested | wants_proposal | objection | pricing | not_interested | already_customer | unknown>", "reasoning": "..." }`;
    try {
        ...
        const parsed = parseLLMContent(result.choices[0].message.content);
...
export async function runCheckReplies(task) {
...
export async function runSendDemoPacket(task) {
    const payload = task.payload;
...
Return JSON: { "subject": "...", "body": "..." }`;
    ...
    const parsed_demo = parseLLMContent(result.choices[0].message.content);
    const subject = parsed_demo.subject ?? `AuthiChain for ${leadOrg ?? segment}: How It Works`;
    const body = parsed_demo.body ?? '';
    if (!body)
        throw new Error('Demo packet LLM returned empty body');
...
export async function runGenerateProposal(task) {
    const payload = task.payload;
    const { leadEmail, leadName, leadOrg, leadTitle, segment, replyText, threadId } = payload;
...
Return JSON: { "subject": "Proposal: AuthiChain Pilot for [Org]", "body": "..." }`;
    ...
    const parsed_proposal = parseLLMContent(result.choices[0].message.content);
    ...
    if (!proposalContent)
        throw new Error('Proposal LLM returned empty content');
...
export async function runSendContract(task) {
    const payload = task.payload;
...
Return JSON: { "subject": "AuthiChain Service Agreement — [Org]", "body": "..." }`;
    ...
    const parsed_contract = parseLLMContent(result.choices[0].message.content);
...
export async function runAutoReply(task) {
    const payload = task.payload;
...
Return JSON: { "subject": "Re: [keep thread subject]", "body": "..." }`;
    ...
    const parsed_reply = parseLLMContent(result.choices[0].message.content);
