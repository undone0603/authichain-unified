...
import { selectTone, SEGMENT_PRIORS, betaMean, betaCI, bayesianPreamble, } from '../_core/bayesian.js';
import { assessRecipient, canSend, domainAcceptsMail, } from '../outreach/send-guard.js';
...
export async function runOutboundEmail(task) {
...
Return JSON: { "subject": "...", "body": "..." }`;
    ...
    const parsed_email = parseLLMContent(result.choices[0].message.content);
    const subject = parsed_email.subject ?? subjectFallback;
    const body = parsed_email.body ?? '';
    if (!body)
        throw new Error('LLM returned empty email body');
    ...
    if (!payload.leadEmail)
        throw new Error('No leadEmail in payload for direct send');
    ...
    const source = payload.verificationSource ?? 'unknown';
