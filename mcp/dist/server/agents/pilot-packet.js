...
export async function runBuildPilotPacket(task) {
...
Return JSON: { "title": "...", "sections": [{ "heading": "...", "content": "..." }] }`;
    ...
    const packet = parseLLMContent(result.choices[0].message.content);
...
export async function runDraftIntelDossier(task) {
    const payload = task.payload;
    const segment = payload.segment ?? 'GOV';
...
Return JSON: { "title": "...", "sections": [{ "heading": "...", "content": "..." }] }`;
    ...
    const dossier = parseLLMContent(result.choices[0].message.content);
