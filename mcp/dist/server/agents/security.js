...
export async function runSecurityAudit(task) {
...
Return JSON:
{
  ...
  `;
    ...
    const auditResult = parseLLMContent(result.choices[0].message.content);
    ...
    if (auditResult.findings.some(f => f.severity === 'CRITICAL' || f.severity === 'HIGH')) {
        await createSystemNotification(1, // Admin user ID
        "🚨 Critical Security Findings", `The Security Audit identified ${auditResult.findings.filter(f => f.severity === 'CRITICAL').length} critical issues. Action required immediately.`, "alert", "/admin/security");
    }
}
