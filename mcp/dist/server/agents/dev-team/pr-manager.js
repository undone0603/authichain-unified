...
import { createPR, getPR, getPRFiles, addPRReview, addPRComment, mergePR, } from './github-service.js';
// ─── OPEN_PR ─────────────────────────────────────────────────────────────
export async function runOpenPR(task) {
...
export async function runCodeReview(task) {
    const p = task.payload;
    ...
    const fileDiffs = changedFiles.slice(0, 10).map(f => `### ${f.filename} (${f.status})\n\`\`\`diff\n${f.patch ?? '(binary or large file)'}\n\`\`\``).join('\n\n');
    ...
    const review = parseLLMContent(result.choices[0].message.content);
