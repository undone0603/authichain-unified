...
import { createBranch, getFile, writeFile, searchCode, } from './github-service.js';
...
// ─── PLAN_SPRINT ─────────────────────────────────────────────────────────
export async function runPlanSprint(task) {
...
- Keep each WRITE_CODE task focused on 1-3 files maximum`;
    ...
    const plan = parseLLMContent(result.choices[0].message.content);
    ...
    await db.insert(missionTasks).values(allTasks.map((t, i) => ({
...
// ─── WRITE_CODE ───────────────────────────────────────────────────────────
export async function runWriteCode(task) {
    const p = task.payload;
    ...
    for (const path of filesToModify) {
        ...
        if (file)
            existingFiles.push({ path, content: file.content });
    }
    ...
    if (!filesToModify.length && !filesToCreate.length) {
        ...
        for (const r of searchResults.slice(0, 4)) {
            ...
            if (file)
                existingFiles.push({ path: r.path, content: file.content });
        }
    }
    ...
    const fileContext = existingFiles.map(f => `### ${f.path}\n\`\`\`typescript\n${f.content}\n\`\`\``).join('\n\n');
...
Write the code changes. Return the full JSON response as specified in your system prompt.`;
    ...
    const codeResult = parseLLMContent(result.choices[0].message.content);
