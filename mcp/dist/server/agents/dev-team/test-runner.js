...
import { getPR, waitForCIRun, createIssue, searchCode, getBranchSha, createBranch, } from './github-service.js';
// ─── RUN_TESTS ────────────────────────────────────────────────────────────
...
// ─── MONITOR_DEPLOY ───────────────────────────────────────────────────────
export async function runMonitorDeploy(task) {
    const p = task.payload;
    ...
    if (!deployHealthy) {
        ...
        await Promise.all(adminIds.map(adminId => createSystemNotification(adminId, 'Deploy Health Check Failed', `Post-deploy health check failed after PR #${p.prNumber}: ${lastError}. AUTO_FIX queued.`, 'alert', issue.html_url)));
    }
}
// ─── FILE_BUG ─────────────────────────────────────────────────────────────
export async function runFileBug(task) {
    const p = task.payload;
    const issue = await createIssue({
        title: p.title,
        body: p.body,
...
export async function runAutoFix(task) {
    const p = task.payload;
    ...
    const diagnosis = parseLLMContent(result.choices[0].message.content);
