...
async function getAuthToken() {
    if (!docusign)
        return "simulated_token_head_only";
    const apiClient = new docusign.ApiClient();
    apiClient.setBasePath(DOCUSIGN_CONFIG.basePath);
    const results = await apiClient.requestJWTUserToken(DOCUSIGN_CONFIG.integrationKey, DOCUSIGN_CONFIG.userId, ['signature', 'impersonation'], Buffer.from(DOCUSIGN_CONFIG.privateKey), 3600);
