...
export async function calculateHarmony() {
    ...
    const [totalAuths] = await d.select({ count: sql `count(*)` }).from(authentications);
    const [anchoredAuths] = await d.select({ count: sql `count(*)` }).from(authentications).where(eq(authentications.blockchainVerified, 1));
    ...
    const [recentAuths] = await d.select({ count: sql `count(*)` }).from(authentications).where(gte(authentications.createdAt, last24h));
    ...
    const [totalStaked] = await d.select({ sum: sql `sum(amount)` }).from(stakingPositions).where(eq(stakingPositions.status, "active"));
