...
vi.mock("./db", async (importOriginal) => {
    ...
    return {
        ...
        getUserNotifications: vi.fn(async (userId, limit = 50) => {
            return store.notifications.filter((n) => n.userId === userId).slice(0, limit);
        }),
        getUnreadNotificationCount: vi.fn(async (userId) => {
            return store.notifications.filter((n) => n.userId === userId && n.isRead === 0).length;
        }),
        markNotificationRead: vi.fn(async (id, userId) => {
            const n = store.notifications.find((n) => n.id === id && n.userId === userId);
            if (n)
                n.isRead = 1;
        }),
        markAllNotificationsRead: vi.fn(async (userId) => {
            store.notifications.filter((n) => n.userId === userId).forEach((n) => { n.isRead = 1; });
        }),
        deleteNotification: vi.fn(async (id, userId) => {
            const idx = store.notifications.findIndex((n) => n.id === id && n.userId === userId);
            if (idx >= 0)
                store.notifications.splice(idx, 1);
        }),
        createLead: vi.fn(async (_data) => ({ id: store.nextLeadId() })),
        // certificates
...
function createAuthContext(role = "user") {
    const user = {
        id: 1,
...
describe("AuthiChain Unified Platform Routers", () => {
    ...
    describe("notifications router", () => {
        ...
        it("notifications.create then list shows the notification", async () => {
            ...
            const found = list.find((n) => n.title === "Visible Test");
    ...
    describe("character", () => {
        it("requires auth for character generation", async () => {
            ...
            await expect(caller.character.generate({ archetype: "guardian" })).rejects.toThrow();
        });
        it("requires auth for agent creation", async () => {
            const caller = appRouter.createCaller(createPublicContext());
            await expect(caller.character.createAgent({ characterAssetId: 1, agentName: "TestAgent", agentType: "guardian" })).rejects.toThrow();
        });
        ...
        it("validates archetype input on generate", async () => {
            const caller = appRouter.createCaller(createAuthContext());
            await expect(caller.character.generate({ archetype: "invalid_archetype" })).rejects.toThrow();
        });
    });
    describe("analytics", () => {
