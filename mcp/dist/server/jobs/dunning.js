...
import { createSystemNotification, hasDunningStepLogged, listPastDueSubscriptions, logActivity, getUserById, } from "../db";
...
async function runStep(subscription, step, message) {
    ...
    if (alreadyLogged)
        return false;
    await createSystemNotification(subscription.userId, "Billing Reminder", message, "alert", "/subscriptions");
...
export async function runDunningEscalation() {
    ...
    for (const sub of pastDue) {
        ...
        if (ageDays >= 14) {
            const didSend = await runStep(sub, "day_14", "Final billing reminder: update payment details to avoid service downgrade.");
            if (didSend)
                sent++;
            continue;
        }
        if (ageDays >= 7) {
            const didSend = await runStep(sub, "day_7", "Billing reminder: payment is still overdue. Please update billing details.");
            if (didSend)
                sent++;
            continue;
        }
        if (ageDays >= 3) {
            const didSend = await runStep(sub, "day_3", "Billing reminder: we could not process your payment. Please update your card.");
            if (didSend)
                sent++;
        }
    }
