...
const { state } = vi.hoisted(() => ({ state: { subs: [], logged: new Set() } }));
const sendEmail = vi.fn(async (..._a) => { });
const createSystemNotification = vi.fn(async (..._a) => { });
const logActivity = vi.fn(async (..._a) => { });
vi.mock('../db', () => ({
    listPastDueSubscriptions: async () => state.subs,
    hasDunningStepLogged: async (id, step) => state.logged.has(`${id}:${step}`),
    createSystemNotification: (...a) => createSystemNotification(...a),
    logActivity: (...a) => logActivity(...a),
    getUserById: async (id) => ({ id, email: `u${id}@example.com`, name: 'User' }),
}));
vi.mock('../email-service', () => ({ sendEmail: (...a) => sendEmail(...a) }));
