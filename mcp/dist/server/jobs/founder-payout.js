...
const clampPct = (n, fallback) => Number.isFinite(n) && n >= 0 && n <= 100 ? n : fallback;
 ...
 */
export function computePayout(grossCents, split = defaultSplit()) {
 ...
 */
export async function runFounderPayout(grossCents) {
    ...
    try {
        await notifyOwner({
            ...
            content: `Gross ${dollars(plan.grossCents)} → Founder ${dollars(plan.founderCents)}, ` +
...
export async function collectedRevenueCents(start, end) {
    const rows = await getRevenueAnalytics(start, end);
    const dollarsTotal = rows.reduce((sum, r) => sum + (parseFloat(r?.amount ?? "0") || 0), 0);
    return Math.round(dollarsTotal * 100);
}
/** Monthly entry point: read last month's collected revenue and run the split. */
export async function runMonthlyFounderPayout(now = new Date()) {
