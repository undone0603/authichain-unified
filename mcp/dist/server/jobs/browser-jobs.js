...
import { runBrowseCompetitorMonitor, runBrowseScrapeIndustryNews, } from '../agents/browser.js';
...
export async function runBrowserAgentJobs() {
    ...
    if (batchRanThisWeek) {
        ...
        for (const competitor of COMPETITORS) {
            try {
                const fakeTask = { id: 0, missionId: 0, payload: competitor };
    ...
    for (const keyword of NEWS_KEYWORDS) {
        ...
        try {
            const payload = { keyword, enqueueNewsTask: true };
            const fakeTask = { id: 0, missionId: 0, payload };
