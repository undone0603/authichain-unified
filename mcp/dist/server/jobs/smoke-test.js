...
async function runSmokeTest() {
    ...
    let health = { integrations: {}, blockers: [] };
    try {
    ...
    const statusIcon = (res) => (res?.connected ? "✅" : res?.configured ? "⚠️ (Configured but not connected)" : "❌ (Not Configured)");
    ...
    try {
        ...
        if (testAi.choices[0].message.content?.includes("LLM_OK")) {
