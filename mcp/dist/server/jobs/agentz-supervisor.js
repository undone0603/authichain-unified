...
export async function runAgentZSupervisor(options) {
    if (!ENV.autonomousPipelineEnabled) {
        throw new Error("AgentZ requires AUTONOMOUS_PIPELINE_ENABLED=true. Set it in runtime environment before starting.");
    }
    ...
    try {
        ...
        throw new Error(`AgentZ supervisor is already running or lock file is stale (${lockFilePath}). Remove the lock file if no process is active.`);
    }
    await logActivity({
        userId: null,
    ...
    try {
        do {
            ...
            try {
                const tickResult = await runSingleTick();
