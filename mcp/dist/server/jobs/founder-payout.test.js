...
const notifyOwner = vi.fn(async (..._a) => { });
const { rev } = vi.hoisted(() => ({ rev: { rows: [] } }));
vi.mock('../db', () => ({
    logActivity: (...a) => logActivity(...a),
    getRevenueAnalytics: async () => rev.rows,
}));
vi.mock('../_core/notification', () => ({ notifyOwner: (...a) => notifyOwner(...a) }));
import { computePayout, defaultSplit, runFounderPayout, lastMonthRange, collectedRevenueCents, runMonthlyFounderPayout, } from './founder-payout';
afterEach(() => {
...
describe('collectedRevenueCents', () => {
    it('sums revenueRecords.amount (dollars) into cents', async () => {
        rev.rows = [{ amount: '199.00' }, { amount: '499.50' }, { amount: 'bad' }];
