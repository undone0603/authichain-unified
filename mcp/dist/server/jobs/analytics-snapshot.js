...
import { getAcceptanceCriteriaStatus, getAdminDashboardMetrics, getBudgetStatus, getFunnelBySegmentAndChannel, getLeadCohorts, getQuarterlyValueReport, getRevenueAnalytics, getWeeklyRevenueDigest, logActivity, } from "../db";
function startOfNDaysAgo(days) {
    const now = new Date();
    return new Date(now.getTime() - days * 24 * 60 * 60 * 1000);
}
export async function runAnalyticsSnapshot() {
    const now = new Date();
    const [metrics, weeklyDigest, budgetStatus, acceptanceStatus, funnel, cohorts, quarterly, revenue30d] = await Promise.all([
