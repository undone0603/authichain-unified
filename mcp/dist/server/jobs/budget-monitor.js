...
export async function runBudgetMonitor() {
    ...
    for (const c of checks) {
        for (const t of [90, 70]) {
            if (c.pct < t)
                continue;
            const action = alertAction(c.metric, t, c.period);
            if (await alreadyAlerted(action))
                continue;
            const count = await notifyAdmins(`Budget Alert: ${c.metric.toUpperCase()} ${t}%`, `${c.metric} spend reached ${c.pct}% for period ${c.period}.`, { metric: c.metric, threshold: t, pct: c.pct, period: c.period });
            await logActivity({
                userId: null,
                action,
                entityType: "budget",
