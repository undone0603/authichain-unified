...
server.tool("verify_authenticity", { certificateNumber: z.string().describe("The unique AuthiChain certificate ID") }, async ({ certificateNumber }) => {
    const cert = await getCertificateByNumber(certificateNumber);
    if (!cert)
        return { content: [{ type: "text", text: "Certificate not found. This product is UNVERIFIED." }] };
...
server.tool("classify_product", {
    ...
    return {
        content: [{ type: "text", text: `CLASSIFIED: Product mapped to industry vertical: ${industry.name}. Confidence: HIGH. Suggested workflow: ${industry.workflow.map((w) => w.name).join(" -> ")}` }]
    };
});
// Tool: Verify Sovereign Deal
server.tool("verify_sovereign_deal", {
    truemarkId: z.string().describe("The TrueMark ID of the deal to verify")
}, async ({ truemarkId }) => {
    const cert = await getCertificateByNumber(truemarkId);
    if (!cert)
        return { content: [{ type: "text", text: "NOT FOUND: This TrueMark ID does not exist in the sovereign ledger." }] };
    const product = await getProductById(cert.productId);
    return {
