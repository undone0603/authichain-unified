...
import { mintAuthenticationNFT } from "../thirdweb";
...
export const nftRouter = router({
    ...
    mintVoyageBloom: protectedProcedure.input(z.object({
        ...
        if (!ENV.walletPrivateKey) {
            throw new TRPCError({ code: "INTERNAL_SERVER_ERROR", message: "Minting wallet not configured" });
        }
        ...
        const result = await mintAuthenticationNFT({
            ...
            privateKey: ENV.walletPrivateKey,
        });
        // Log in DB and trigger event
        await db.logActivity({
            userId: ctx.user.id,
            action: "nft_minted_voyage_bloom",
            entityType: "nft",
            entityId: 0 // Will link to transaction hash in metadata or separate table
        });
        return result;
    }),
    collections: router({
    ...
    auctions: router({
        ...
        create: protectedProcedure.input(z.object({
            ...
            if (!nft)
                throw new TRPCError({ code: "NOT_FOUND", message: "NFT not found" });
            if (nft.ownerId !== ctx.user.id)
                throw new TRPCError({ code: "FORBIDDEN", message: "You do not own this NFT" });
        ...
        bid: protectedProcedure.input(z.object({
            ...
            if (auction.status !== "active")
                throw new TRPCError({ code: "BAD_REQUEST", message: "Auction not active" });
