...
export const authenticateRouter = router({
    analyze: protectedProcedure.input(z.object({
        ...
        if (quotaResult === "exceeded")
            throw new TRPCError({ code: "FORBIDDEN", message: "Monthly quota exceeded. Please upgrade your plan." });
        ...
        const aiResult = parseLLMContent(response.choices[0].message.content);
        ...
        try {
            ...
            await db.createSystemNotification(ctx.user.id, `Authentication ${emoji}: ${product.name}`, `${product.brand || "Product"} ${product.name} scored ${aiResult.confidence}% confidence as ${aiResult.result}. ${aiResult.recommendation}`, aiResult.result === "counterfeit" ? "alert" : "authentication", "/authenticate");
        }
        catch (notifErr) {
