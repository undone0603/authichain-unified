...
import { provisionVendorAccount, generateOnboardingLink, createVendorCheckoutSession, createPlatformSubscriptionPlan, attachBalancePaymentMethod, subscribeVendorToPlatform, } from "./stripe-connect-service";
export const stripeConnectRouter = router({
    provisionAccount: protectedProcedure
        ...
        const accountId = await provisionVendorAccount(ctx.user.id, ctx.user.name ?? "Vendor", ctx.user.email ?? "", input.country);
        return { accountId };
    }),
    ...
    subscribe: protectedProcedure
        ...
        .mutation(async ({ input }) => {
        const subscription = await subscribeVendorToPlatform(input.accountId, input.paymentMethodId, input.priceId);
