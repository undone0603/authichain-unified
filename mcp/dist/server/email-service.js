...
function suppressionSet() {
    return new Set(ENV.suppressionList
        .split(",")
...
export async function checkThreadReplies(threadId) {
    const gmailAccessToken = await getGmailAccessToken();
    if (!gmailAccessToken || !threadId)
        return { hasReply: false };
    const res = await fetch(`https://gmail.googleapis.com/gmail/v1/users/me/threads/${threadId}?format=full`, { headers: { Authorization: `Bearer ${gmailAccessToken}` } });
    if (!res.ok)
        return { hasReply: false };
    ...
    const replies = messages.filter(m => Array.isArray(m.labelIds) && m.labelIds.includes("INBOX"));
    if (replies.length === 0)
        return { hasReply: false };
