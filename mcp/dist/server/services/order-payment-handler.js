...
export async function handleServiceOrderPayment(session) {
    ...
    if (order.userId) {
        await db.createSystemNotification(order.userId, "Payment confirmed", `Your service order for ${order.serviceType} is paid and will be fulfilled shortly.`, "success", "/orders");
    }
