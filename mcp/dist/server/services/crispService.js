...
export async function sendCertificateEmail(params) {
    ...
    const { to, customerName, certificateNumber, productName, tier, isAuthentic, confidenceScore, certificateUrl, nftTokenId, } = params;
    ...
    try {
        ...
        await crispClient.website.sendMessageInConversation(config.websiteId, to, // session_id (email)
        {
            type: 'text',
            from: 'operator',
            origin: 'chat',
...
function generateCertificateMessage(params) {
    const { customerName, certificateNumber, productName, tier, isAuthentic, confidenceScore, certificateUrl, nftTokenId, } = params;
...
export async function sendCrispMessage(params) {
    ...
    try {
        await crispClient.website.sendMessageInConversation(config.websiteId, params.email, {
            type: 'text',
            from: 'operator',
            origin: 'chat',
...
export async function updateCrispProfile(params) {
    ...
    try {
        ...
        if (params.name) {
            await crispClient.website.updatePeopleProfile(config.websiteId, params.email, {
                person: {
        ...
        // Update custom data
        if (params.data) {
            await crispClient.website.updatePeopleData(config.websiteId, params.email, params.data);
        }
