 ...
 */
export async function generateProductAudioStory(data) {
    ...
    for (const endpoint of endpoints) {
        try {
            ...
            if (response.ok) {
                ...
                const { url } = await storagePut(`audio/stories/${Date.now()}-${data.brandName.toLowerCase().replace(/\s+/g, '_')}.mp3`, Buffer.from(buffer), "audio/mpeg");
                return url;
            }
