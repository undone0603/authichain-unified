 ...
 */
export async function provisionVendorAccount(userId, displayName, email, countryCode = 'US') {
    ...
    try {
        ...
        throw new Error("Stripe V2 provisionVendorAccount: unparseable response body");
    }
 ...
 */
export async function generateOnboardingLink(vendorAccountId) {
    const stripe = getStripe();
    ...
    try {
        ...
        throw new Error("Stripe V2 generateOnboardingLink: unparseable response body");
    }
    return body.url;
}
/**
 ...
 */
export async function createVendorCheckoutSession(vendorAccountId, currency = 'usd') {
    const stripe = getStripe();
 ...
 */
export async function attachBalancePaymentMethod(vendorAccountId) {
    const stripe = getStripe();
 ...
 */
export async function subscribeVendorToPlatform(vendorAccountId, paymentMethodId, priceId) {
    const stripe = getStripe();
    try {
