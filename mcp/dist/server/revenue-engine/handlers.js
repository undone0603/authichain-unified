...
async function supabaseUpsert(table, row, conflictKey = "stripe_subscription_id") {
    const SUPABASE_URL = process.env.SUPABASE_URL;
    const SERVICE_ROLE = process.env.SUPABASE_SERVICE_ROLE_KEY;
    const res = await fetch(`${SUPABASE_URL}/rest/v1/${table}`, {
        method: "POST",
...
export async function handleVerificationEvent(ev) {
    try {
...
export async function handleCertificateMint(ev) {
    try {
        const certRow = {
...
export async function handleDispensaryScan(ev) {
    try {
...
export async function handleSubscriptionCreated(ev) {
    try {
        const row = {
...
export async function handlePaymentFailed(ev) {
    try {
