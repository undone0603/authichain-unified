...
export function createInternalRouter() {
    ...
    router.post("/verify", async (req, res) => {
        try {
            ...
            if (!lookupId)
                return res.status(400).json({ error: "identifier, productId, or barcode required" });
    ...
    router.post("/certificates/verify", async (req, res) => {
        try {
            ...
            if (!number)
                return res.status(400).json({ error: "certNumber body field required" });
            if (number.length > 64)
    ...
    router.post("/cannabis/verify", async (req, res) => {
        try {
            ...
            if (!strain)
                return res.status(400).json({ error: "strainName or strainId required" });
            const metadata = {
                name: strain,
                type: "HYBRID",
                genetics: ["Unknown"],
    ...
    router.post("/usage/report", async (req, res) => {
        try {
            ...
            await Promise.all(records.map((r) => reportUsageToStripe(r.tenantId, r.endpoint, r.count)));
    ...
    router.get("/tenant", async (req, res) => {
        try {
            ...
            if (!apiKey)
                return res.status(400).json({ error: "Authorization: Bearer <apiKey> required" });
