 ...
 */
export function calculateStrainRarity(metadata, profile) {
    let score = 50; // Base score
    // THC Thresholds
    if (profile.thc > 30)
        score += 25;
    else if (profile.thc > 25)
        score += 15;
    else if (profile.thc > 20)
        score += 10;
    // CBD Rarity
    if (profile.cbd > 10)
        score += 20;
    // Genetics Complexity
    if (metadata.genetics.length > 2)
        score += 10;
 ...
 */
export function formatTruthLayerMetadata(metadata, profile) {
 ...
 */
export function getStrainColor(type) {
    const colors = {
