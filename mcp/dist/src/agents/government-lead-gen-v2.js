...
const ACTIVE_ENTITY = {
    ...
    mission: 'Government Document & Product Verification — American-built infrastructure for procurement transparency and lives saved globally.',
};
...
async function findSupabaseOpportunities(queryText, limit = 8) {
    ...
    try {
        ...
        console.warn('[gov-engine] vector match skipped:', err.message);
        return [];
    }
}
// ──────────────────────────────────────────────────────────────────────────────
...
function rollingWindow(days) {
    ...
    const sam = (d) => `${pad(d.getMonth() + 1)}/${pad(d.getDate())}/${d.getFullYear()}`;
...
async function fetchSAMOpportunities() {
    ...
    const q = encodeURIComponent('counterfeit prevention OR Buy American OR traceability OR document authentication OR supply chain verification');
...
// ──────────────────────────────────────────────────────────────────────────────
async function fetchSAMContractAwards() {
    const key = process.env.SAM_GOV_API_KEY;
    if (!key) {
    ...
    const url = `https://api.sam.gov/contract-awards/v1/search` +
        `?api_key=${key}&q=${q}&limit=15` +
...
// ──────────────────────────────────────────────────────────────────────────────
async function fetchUSASpendingAwards(agencyName) {
...
async function runFiveAgentConsensus(lead) {
...
async function generateQRON(payload) {
    try {
        ...
        if (res.ok) {
            ...
            if (imageUrl)
                return { signature: imageUrl, payload, style: 'marble_white' };
        }
    }
...
async function sendOutreach(lead, proposal, qron) {
    if (DRY_RUN) {
    ...
    const contactEmail = poc?.email || lead.raw?.email;
    if (!contactEmail) {
...
async function mintPilotNFT(args) {
    if (DRY_RUN) {
        console.log(`[gov-engine] DRY_RUN mint skipped — would mint pilot NFT for ${args.agency} (lead ${args.leadId}, score ${args.consensusScore}, sig ${args.qronSignature})`);
        return;
    }
    // On-chain mint not yet implemented — skip gracefully so live runs don't crash.
    console.warn(`[gov-engine] mintPilotNFT: on-chain mint not wired — skipping for lead ${args.leadId} (${args.agency}). ` +
...
export async function runAdvancedGovernmentLeadGen() {
    console.log(`🚀 GovChain autonomous lead-gen v2.5 starting (DRY_RUN=${DRY_RUN}) — SAM Opps + Contract Awards v1 + USAspending + pgvector`);
    ...
    console.log(`[gov-engine] SAM.gov: ${opportunities.length} opportunities, ${awards.length} contract awards`);
    ...
    for (const lead of allLeads) {
        ...
        processed++;
        console.log(`✅ Processed ${lead.agency} — consensus ${consensus.score}, qron ${qron.signature}`);
    }
    console.log(`🎯 ${processed}/${allLeads.length} federal leads processed (DRY_RUN=${DRY_RUN}, signer=${ACTIVE_ENTITY.wallet})`);
