...
async function viaGmail(args, key) {
    const user = process.env.GMAIL_USER;
    if (!user)
        return { ok: false, provider: 'gmail', error: 'GMAIL_USER not set' };
    try {
...
async function viaResend(args, key) {
...
async function viaBrevo(args, key) {
...
async function viaSendGrid(args, key) {
    const sender = parseFrom(args.from);
...
export async function sendEmail(args) {
    ...
    let last = { ok: false, provider: 'none', error: 'no provider configured' };
