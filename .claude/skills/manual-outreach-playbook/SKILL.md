...
This repo already has a fully-built automated outreach pipeline (`workers/qron-outreach/src/index.ts`'s `DPP_QUEUE`, 53+ AgentZ workflows, an LLM personalizer, Stripe SKUs wired end-to-end). It is **deliberately disabled**. `docs/outreach-deliverability-runbook.md` documents why: zero replies ever, across every automated campaign, at a 16% bounce rate, because contacts were guessed (`compliance@company.com`-style patterns) rather than real, and (per the runbook's initial 2026-08-07 diagnosis) sends were failing before copy or targeting could even matter. The runbook's 2026-08-16 update found the real root cause was credential wiring, not DNS -- see the note at the end of this file before assuming domain authentication is still an open item.
...
This skill exists to run that manual phase well and leave a record of what worked. **Do not** use it to justify re-enabling `dpp-outreach-trigger.yml`, `outreach-trigger.yml`, or any batch/cron send. If asked to "automate this" or "turn this into a routine," point back to this section and to the runbook's success criteria (bounce rate under 3% on a verified list, a monitored reply-to, at least one message proven to get real replies by hand -- domain authentication itself is already done, see the note at the end of this file) before agreeing.
...
Only after several of these cycles produce real replies with a consistent message -- per the runbook's own bar -- does it become worth revisiting automation. **Domain authentication is not part of that remaining work**: `authichain.com` is already fully authenticated (SPF/DKIM/DMARC), just credentialed on a second Resend account (`RESEND_API_KEY2`) that a single-key client didn't know existed -- see `docs/outreach-deliverability-runbook.md`'s 2026-08-16 update. Do not attempt to re-authenticate or re-add that domain to another account; Resend will refuse it, and forcing it would reissue DKIM keys and break the account that legitimately holds it. The real remaining prerequisites before re-automating are: confirm the sender-preflight/credential-resolution fix from that update is still wired up, verify contacts at scale (not guessed), and warm up send volume gradually with human review on. Automating a message that hasn't been proven by hand just automates the silence.
