...
for (const file of fixtures) {
  ...
  const sourceIds = new Set(identity.sources.map((s: {source_id:string}) => s.source_id));
  const claimIds = new Set(identity.claims.map((c: {claim_id:string}) => c.claim_id));
  for (const claim of identity.claims) {
    const supported = identity.evidence_edges.some((e: {from:string,to:string,relation:string}) =>
