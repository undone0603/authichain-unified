...
const CHANNELS: Array<{ name: string; category: string; daily_cap: number; description: string }> = [
  ...
  { name: 'email.qron-outreach', category: 'email', daily_cap: 10, description: 'workers/qron-outreach (Cloudflare Worker) QRON + EU DPP cold outreach — integrated with this guardrail (check/record on every send, bounce webhook calls /suppress). Seeded disabled; flip on when ready to actually send.' },
