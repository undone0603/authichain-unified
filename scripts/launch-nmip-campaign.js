    ...
    for (const lead of NMIP_CAMPAIGN) {
      ...
      const [newLead] = await sql`
        ...
        VALUES (
          ...
          'hot',
          98,
          ${JSON.stringify({ sector: lead.target_sector, use_case: lead.use_case, narrative: lead.narrative, pilot_id: 'NMIP-2026-V1' })}
        )
        RETURNING id
      `;
      await sql`
        INSERT INTO automation_logs (workflow_name, trigger_type, status, payload)
        VALUES ('nmip_outreach_sequence', 'manual', 'success', ${JSON.stringify({ target: lead.company, location: lead.location, narrative_sent: lead.narrative, lead_id: newLead.id })})
      `;
    }
