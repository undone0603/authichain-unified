...
const ACRONYMS = { qr: 'QR', eu: 'EU', us: 'US', gs1: 'GS1', epcis: 'EPCIS', dscsa: 'DSCSA', ppwr: 'PPWR', untp: 'UNTP' };
const titleCase = (s) =>
  s.split(/\b/).map((w) => {
...
const DATA = [
  ...
  // ── standards & regulatory deadlines (2026 research, round 2) ─────
  { keyword: 'ppwr digital labelling qr code', brand: 'authichain', schemaType: 'Service',
    lead: 'PPWR (EU) 2025/40 requires a QR code or standardized digital data carrier on packaging with sorting and material information from August 2026, and a full Digital Product Passport alongside it from August 2028 — the same GS1 Digital Link identifier and signed-record model this protocol already issues.',
    bullets: ['The QR/digital carrier supplements the printed label under PPWR — it is not allowed to replace it, and the record behind it can be the same signed, anchored format used for product authentication', 'General PPWR provisions bind from 12 August 2026; the combined packaging DPP plus standardized QR labelling requirement follows on 12 August 2028', 'One GS1 Digital Link identifier per packaging SKU works for both the near-term sorting-data QR and the later DPP resolution — no second code to add in 2028'],
    faqs: [{ q: 'Does the QR code replace the printed recycling label?', a: 'No — PPWR requires the digital carrier to supplement the mandatory printed label, not substitute for it.' }, { q: 'What is the deadline?', a: 'Most PPWR provisions, including the digital labelling requirement, become binding on 12 August 2026; the packaging Digital Product Passport requirement follows on 12 August 2028.' }] },
  { keyword: 'untp digital product passport', brand: 'authichain', schemaType: 'Service',
    lead: 'UNTP — the UN Transparency Protocol — is targeting a 1.0 Digital Product Passport specification release on 1 September 2026, built on the same W3C Verifiable Credentials Data Model 2.0 this protocol’s records already use.',
    bullets: ['UNTP issues every credential, including its Digital Product Passport, as a W3C VC 2.0 object — the identical base data model documented in SPEC.md §2, not a competing format', 'UNTP is UN/CEFACT and UNECE work, currently on 0.6.x pilot releases; SPEC.md is a v0.1.0 draft — both are pre-1.0 specs published for implementers to compare, not finished standards', 'The 28-fixture conformance suite and reference verifier are public, so the gap between a ProvenanceRecord and a UNTP DigitalProductPassport credential is something an implementer can check directly, not take on faith'],
    faqs: [{ q: 'Is a record issued under this protocol a valid UNTP DigitalProductPassport credential?', a: 'Not directly — UNTP defines its own credential type and JSON-LD context on top of VC 2.0. This protocol’s ProvenanceRecord shares the base standard but is a different schema; mapping between them is inspectable, not assumed.' }, { q: 'When does UNTP 1.0 ship?', a: 'The UNTP team is targeting a 1.0 release on 1 September 2026; current releases are versioned 0.6.x and marked suitable for pilot implementations.' }] },
  { keyword: 'battery passport due diligence requirement', brand: 'authichain', schemaType: 'Service',
    lead: 'The EU Battery Regulation’s due-diligence obligation — sourcing checks on cobalt, lithium, nickel, and natural graphite — was postponed by Regulation (EU) 2025/1561 from 18 August 2025 to 18 August 2027, separate from the battery passport’s own 18 February 2027 deadline.',
    bullets: ['Regulation (EU) 2025/1561, published 30 July 2025, is the amendment that moved the due-diligence start date two years — it does not delay the battery passport itself', 'Due diligence is a raw-material sourcing report; the battery passport carries carbon footprint, state-of-health, and material-recovery data on the same GS1 Digital Link and VC 2.0 base this protocol specifies — two obligations under one regulation, on two different dates', 'A sourcing claim in a due-diligence report can be issued as its own signed Verifiable Credential and hashed to an anchor, so the claim is checkable independent of the PDF it started as'],
    faqs: [{ q: 'Did the due-diligence deadline change?', a: 'Yes — Regulation (EU) 2025/1561, published 30 July 2025, postponed it from 18 August 2025 to 18 August 2027.' }, { q: 'Does the battery passport already satisfy due diligence?', a: 'No — they are separate obligations under the same regulation. The passport carries product data; due diligence is a raw-material sourcing report for large economic operators.' }] },
  { keyword: 'digital product passport access rights', brand: 'authichain', schemaType: 'Service',
    lead: 'ESPR splits Digital Product Passport access into tiers — public, professional/economic-operator, and authority — and the registry rule this protocol already implements, a permanent URL with no authentication, is exactly what the public tier requires.',
    bullets: ['Annex XIII and the Article 77(9) implementing act govern which fields each tier sees; the split is defined per product-group delegated act, not by this protocol', 'SPEC.md §6 requires records to resolve from a permanent URL without authentication — verification that requires an account is not public verification, and that is the public tier by definition', 'Field-level restriction for confidential technical data is not in spec v0.1.0 — every field in a record issued today is visible to anyone who resolves its URL, which is the public-tier behavior and not yet the professional or authority tiers'],
    faqs: [{ q: 'Does this protocol already support ESPR-style tiered access?', a: 'Not yet — v0.1.0 issues one record with one visibility level. Tiering confidential fields for repairers, recyclers, or authorities is a real requirement the delegated acts will define, and it is not built here.' }, { q: 'Is a single public record still useful before tiering exists?', a: 'Yes, for the tier ESPR requires to be free and unauthenticated — which is how every record resolves today.' }] },
];
