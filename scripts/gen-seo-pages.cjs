...
const ACRONYMS = { qr: 'QR', eu: 'EU', us: 'US', gs1: 'GS1', epcis: 'EPCIS', dscsa: 'DSCSA', dpp: 'DPP', eudr: 'EUDR', ppwr: 'PPWR', did: 'DID', cen: 'CEN', cenelec: 'CENELEC', espr: 'ESPR', nft: 'NFT', fsma: 'FSMA', iso: 'ISO', sd: 'SD', jwt: 'JWT', dfars: 'DFARS', thc: 'THC', eudi: 'EUDI' };
const titleCase = (s) =>
  s.split(/\b/).map((w) => {
...
const DATA = [
  ...
  { keyword: 'critical raw materials act magnet labeling', brand: 'authichain', schemaType: 'Service',
    lead: 'The Critical Raw Materials Act has required a Commission report on which raw materials need supply-chain labeling since it entered into force in May 2024, and a February 2026 Commission proposal would extend that into a concrete rule: electronics containing permanent magnets would carry a label and a data carrier linked to a unique product identifier, plus disclosed recycled content for magnets over 0.2kg.',
    bullets: ['The proposed data carrier ties directly to a unique product identifier — the same role a signed AuthiChain record already plays for EU DPP electronics, no separate identifier scheme to build', 'Recycled-content disclosure under the proposal covers eight named metals — neodymium, dysprosium, praseodymium, terbium, boron, samarium, nickel, cobalt — recovered from post-consumer waste, a field structure that slots into the same credentialSubject shape already used for battery recycled-content data', 'The base Act’s own 24 November 2026 deadline — a Commission report naming which critical raw materials get a labeling obligation — lands before the magnet-specific proposal is expected to finish co-legislation, so the labeling scope itself is still moving'],
    faqs: [{ q: 'Is the magnet labeling requirement final law?', a: 'Not yet — Council reached a negotiating position in March 2026, but the Article 28/29 magnet provisions are Commission proposals still in the EU’s co-legislative process as of this writing, not an adopted regulation.' }, { q: 'What is definitely required today under the Critical Raw Materials Act?', a: 'The base Act, in force since 23 May 2024, requires the Commission to report by 24 November 2026 on which specific critical raw materials warrant a labeling and traceability obligation — that scoping decision precedes any product-specific rule.' }] },
  { keyword: 'dfars counterfeit electronic parts traceability', brand: 'govchain', schemaType: 'Service',
    lead: 'DFARS 252.246-7007 requires DoD contractors and every subcontractor tier below them to detect and avoid counterfeit electronic parts, with documented traceability back to the original manufacturer — a duty that flows down whether or not a supplier holds a direct DoD contract.',
    bullets: ['Signed provenance records give a subcontractor documented traceability to the original component manufacturer, the exact artifact 252.246-7007 requires a contractor’s detection-and-avoidance system to produce', 'A record verifies offline against its own signature — a prime contractor’s flow-down audit does not require a live database lookup or an account with the component supplier', 'The clause applies to Cost Accounting Standards-covered contractors and flows down through every subcontractor tier, not only to companies holding a direct DoD contract'],
    faqs: [{ q: 'Does DFARS 252.246-7007 apply to us if we don’t hold a DoD contract directly?', a: 'Yes, if you supply electronic parts into a covered contractor’s supply chain — the clause flows down to subcontractors at all tiers under Cost Accounting Standards-covered prime contracts.' }, { q: 'Does a signed provenance record satisfy the detection-and-avoidance system requirement by itself?', a: 'No — 252.246-7007 requires a documented system covering training, inspection, testing, and reporting of suspect parts. A verifiable per-part record supports the traceability element of that system; it isn’t the whole system.' }] },
  { keyword: 'eu customs data hub traceability 2028', brand: 'authichain', schemaType: 'Service',
    lead: 'Council and Parliament agreed the EU customs reform in March 2026, creating a new EU Customs Authority and a Customs Data Hub that goes live for e-commerce goods on 1 July 2028, with all other movements of goods phased in by 1 March 2034 — a single online environment where an importer logs a shipment’s provenance instead of filing one declaration per import.',
    bullets: ['The reform names AI, IoT, and blockchain among the technologies the new Customs Authority expects to use for supply-chain visibility — not a claim invented for this page, but the stated direction of the legislation itself', 'A record that resolves at a permanent, unauthenticated URL is the shape a real-time data feed can query directly, without a portal login sitting between a shipment and an inspector', 'The Data Hub’s 2028 start date is for e-commerce consignments specifically — full coverage of all goods movements phases in over six more years, through March 2034, so this is not one cutover date for every importer'],
    faqs: [{ q: 'Is the Customs Data Hub the same as the EU Digital Product Passport registry?', a: 'No — separate systems on separate legal bases. The DPP registry, live since 20 July 2026, holds product-passport data under the ESPR. The Customs Data Hub is the EU’s new general import-control system under the customs reform, launching for e-commerce goods in 2028.' }, { q: 'Has the customs reform actually become law yet?', a: 'Council and Parliament reached political agreement in March 2026. Formal adoption and publication follow that agreement — check the current status before treating any specific date as final.' }] },
  { keyword: 'section 781 hemp total thc ban 2026', brand: 'strainchain', schemaType: 'Service',
    lead: 'Section 781 of the 2026 Appropriations Act redefines federal hemp at 0.3% total THC — counting THCA and delta-8 — and caps finished products at 0.4mg total THC per container, effective 12 November 2026; state-licensed cannabis programs already test and track total THC the way this new federal standard now requires of hemp.',
    bullets: ['The 0.4mg-per-container cap applies to hemp-derived products under the federal Farm Bill definition, not to cannabis sold through a state-licensed, METRC- or BioTrack-tracked program — the two are regulated on separate tracks', 'A StrainChain record already carries the total-THC lab result Section 781 now requires of hemp products, because state cannabis testing programs measure total THC, not just delta-9, for potency labeling', 'Chain-of-custody events tie a batch’s COA to the unit a customer scans, the same tamper-evident record model used for state recall traceability'],
    faqs: [{ q: 'Does Section 781 affect state-licensed cannabis dispensaries?', a: 'No — it amends the federal Farm Bill definition of hemp. Cannabis sold through a state marijuana program is regulated separately and was already subject to state total-THC testing requirements.' }, { q: 'What happens to hemp-derived THC products after 12 November 2026?', a: 'Products exceeding 0.4mg total THC per container fall outside the federal hemp definition once the provision takes effect. Several bills, including a proposed Cannabinoid Safety and Regulation Act, would authorize interstate commerce under a new framework, but none had passed as of this writing.' }] },
  { keyword: 'eudi wallet digital product passport verification', brand: 'authichain', schemaType: 'Service',
    lead: 'The EU Digital Identity Wallet, mandatory in every member state by the end of 2026 under eIDAS 2.0, and the EU Digital Product Passport are both built on verifiable-credential architecture — but the ESPR does not require a DPP to be read through a wallet, and a wallet does not require a passport to identify its holder; they solve separate trust problems that happen to share a data model.',
    bullets: ['A DPP record resolves from a permanent, unauthenticated GS1 Digital Link URL — any scanner reads it, with or without a wallet installed, matching the ESPR’s requirement that a passport stay accessible for the product’s full lifecycle', 'Both systems are W3C Verifiable Credentials under the hood, so an issuer building DPP records to spec today is not building on an architecture the EUDI Wallet will later obsolete', 'eIDAS 2.0 governs the “verified economic operator” identity check every DPP issuer already has to pass at registration — the wallet standard and the DPP registration process share a legal framework, not a runtime dependency'],
    faqs: [{ q: 'Do we need to integrate with the EUDI Wallet to issue a valid Digital Product Passport?', a: 'No — ESPR requires a passport to resolve from a data carrier like a QR code to a record any conforming reader can verify. The EUDI Wallet is a separate eIDAS 2.0 system for personal identity credentials, not a required reader for product passports.' }, { q: 'When do EU member states have to offer a certified wallet?', a: 'By the end of 2026, per eIDAS 2.0 (Regulation (EU) 2024/1183, in force since 20 May 2024) — each of the 27 member states must provide citizens and residents at least one certified EUDI Wallet.' }] },
];
