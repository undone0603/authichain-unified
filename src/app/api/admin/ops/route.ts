...
/**
 * GET /api/admin/ops
 * Operations dashboard: success/failure aggregation over the last 24h.
...
export async function GET() {
  ...
  try {
    ...
    type Row = { workflow_name: string; status: string; error_message: string | null; payload: string | null; created_at: string };
    ...
    for (const r of allRows) {
      ...
      else if (r.status === 'failure') { entry.failure++; if (!entry.lastError) entry.lastError = r.error_message || '(no message)'; }
    ...
    const failures = allRows.filter(r => r.status === 'failure').slice(0, 50)
      .map(r => ({ workflow: r.workflow_name, error: r.error_message, payload: r.payload, at: r.created_at }));

    const recent = allRows.slice(0, 50).map(r => ({ workflow: r.workflow_name, status: r.status, at: r.created_at }));
    const totals = { success: allRows.filter(r => r.status === 'success').length, failure: allRows.filter(r => r.status === 'failure').length };

    return NextResponse.json({ window_hours: 24, generated_at: new Date().toISOString(), totals, summary, failures, recent });
  } catch (err: unknown) {
