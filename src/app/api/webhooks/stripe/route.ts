...
async function handleCheckoutSessionCompleted(
  ...
  console.log(
    `[stripe-webhook] checkout.session.completed: ${sessionId}, email: ${customerEmail}, amount: ${amountCents}`
  );

  if (!customerEmail) {
    console.warn(
      `[stripe-webhook] No customer_email in checkout session ${sessionId}`
    );
    return;
  }
  ...
  if (updateError) {
    console.error(
      "[stripe-webhook] Failed to update lead status:",
      updateError
    );
...
async function handleInvoicePaymentSucceeded(
  ...
  console.log(
    `[stripe-webhook] invoice.payment_succeeded: ${invoice.id}, subscription: ${subscriptionId}`
  );

  if (!subscriptionId) {
    console.warn(
      `[stripe-webhook] No subscription_id in invoice ${invoice.id}`
    );
    return;
  }
  ...
  if (!customerEmail) {
    console.warn(
      `[stripe-webhook] No customer email found for subscription ${subscriptionId}`
    );
  ...
  if (updateError?.code === "PGRST116" && customerEmail) {
    ...
    if (insertError) {
      console.error(
        "[stripe-webhook] Failed to insert subscription:",
        insertError
      );
      throw insertError;
    }
...
async function handleInvoicePaymentFailed(supabase: AdminSupabase, event: Stripe.Event) {
  ...
  console.log(
    `[stripe-webhook] invoice.payment_failed: ${invoiceId}, subscription: ${subscriptionId}`
  );
  ...
  console.log(
    `[stripe-webhook] Alert created for failed payment: ${invoiceId}`
  );
}
