...
interface AdminStats {
  ...
  governance: {
    total_staked_qron: number;
    total_burned_qron: number;
      ...
      <div className="max-w-7xl mx-auto px-6 py-12">
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-6 mb-12">
          {[
            {
              label: "Global Scans",
              val: stats?.creative?.total_scans?.toLocaleString(),
              icon: Activity,
              color: "text-gold",
            },
            {
              label: "Industrial Assets",
              val: stats?.industrial?.total_products,
              icon: Package,
              color: "text-blue-400",
            },
            {
            ...
            <div
              key={m.label}
              className="protocol-card p-8 bg-zinc-950/50 border-zinc-900 group hover:border-gold/30 transition-all"
            >
              <div className="flex justify-between items-start mb-4">
                <div
                  className={`p-3 rounded-xl bg-zinc-900 border border-zinc-800 ${m.color}`}
                >
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 space-y-8">
                  ...
                  <div className="flex justify-between items-end">
                    <div>
                      <h3 className="text-lg font-black uppercase text-white flex items-center gap-2">
                        QRON.space{" "}
                        <span className="px-2 py-0.5 rounded bg-gold/10 text-gold text-[8px] font-black uppercase">
                          Story Mode Active
                        </span>
                      </h3>
                      <p className="text-xs text-zinc-500 font-bold uppercase tracking-tight">
                        Creative AI Utility & Digital Reveal
                      </p>
                    </div>
                    <p className="text-sm font-black text-gold">
                      {stats?.creative?.total_qrons} Creations
                    </p>
                  </div>
                ...
                <div className="space-y-4">
                  <div className="flex justify-between items-end">
                    <div>
                      <h3 className="text-lg font-black uppercase text-white flex items-center gap-2">
                        StrainChain.io{" "}
                        <span className="px-2 py-0.5 rounded bg-blue-500/10 text-blue-400 text-[8px] font-black uppercase animate-pulse">
                          Watchdog Enabled
                        </span>
                      </h3>
                      <p className="text-xs text-zinc-500 font-bold uppercase tracking-tight">
                        Industrial Provenance & Geo-fencing
                      </p>
                    </div>
                    <p className="text-sm font-black text-blue-400">
                      {stats?.industrial?.total_certifications} Certifications
                    </p>
                  </div>
                  <div className="w-full h-2 bg-zinc-900 rounded-full overflow-hidden">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <Link
                href="/admin/products"
                className="protocol-card p-6 flex items-center justify-between group hover:border-gold/40 transition-all"
              >
          ...
          <div className="space-y-6">
            <section className="protocol-card p-8 border-gold/10 bg-gold/[0.02]">
              ...
              <Link
                href="/admin/leads"
                className="btn-gold w-full py-4 font-black uppercase tracking-widest text-[10px] flex items-center justify-center gap-2"
              >
            ...
            <section className="protocol-card p-8 bg-zinc-950">
              ...
              <div className="space-y-4">
                {stats?.recent_logs && stats.recent_logs.length > 0 ? (
                  ...
                  <p className="text-[10px] text-zinc-700 font-black uppercase text-center py-4">
                    Waiting for Protocol Pulse...
                  </p>
                )}
              </div>
            </section>
