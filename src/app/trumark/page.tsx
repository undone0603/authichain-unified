  ...
  return (
    <div className="min-h-screen bg-white text-slate-900">
      <section className="px-6 pt-28 pb-16">
        <div className="max-w-3xl">
          ...
          <p className="text-slate-600 text-lg leading-relaxed mb-8 max-w-2xl">
            TruMark is the physical mark a shopper or inspector scans. It is not a price.
            A cannabis brand publishes one genetics passport for $49. Origin and EU
            documentation run on EU DPP Readiness at $299. Larger tag programs start
            from the published catalogue or a written packet.
          </p>
          <div className="flex flex-col sm:flex-row gap-3">
            <Link href={PASSPORT} className="inline-flex items-center justify-center rounded-lg bg-indigo-600 px-6 py-3 text-sm font-semibold text-white">
              Passport checkout — $49
            </Link>
            <Link href={DPP} className="inline-flex items-center justify-center rounded-lg border border-slate-300 px-6 py-3 text-sm font-semibold text-slate-900">
              DPP checkout — $299
            </Link>
      ...
      <section className="max-w-7xl mx-auto px-6 pb-20 grid gap-5 md:grid-cols-3">
        {[
          { t: 'TruMark', d: 'The seal and scan story already used in the StrainChain demo and enterprise tag-mint copy. Not a separate Stripe SKU.' },
          { t: 'Passport — $49', d: 'One published genetics passport from existing CoAs. Live GET /api/checkout/plan/strainchain_passport.' },
          { t: 'EU DPP Readiness — $299', d: 'Written readiness assessment and self-serve activation. Live GET /api/checkout/dpp.' },
        ].map((c) => (
      ...
      <section className="border-t border-slate-200 bg-slate-50 px-6 py-16">
        <div className="max-w-2xl">
          <h2 className="text-2xl font-semibold text-slate-950 mb-3">Publish a passport or start DPP</h2>
          <p className="text-sm text-slate-600 mb-6">
            Self-serve Stripe checkout. For an enterprise tag program, email hello@authichain.com
            and ask for the written packet. Async only — no scheduled calls.
          </p>
          <a
