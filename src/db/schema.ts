...
// ─── SEO Pages (generated content) ─────────────────────────────────────────
// Programmatic-SEO landing pages generated by server/agents/seo-content.ts,
// gated by the content.publish guardrail channel. Served dynamically by
// /p/[serial] (force-dynamic route) alongside the statically committed pages
// in content/seo/pages.json — no rebuild needed for a new row to go live.
//
// Named seo_generated_pages, not seo_pages: a DIFFERENT, pre-existing
// seo_pages table already exists (SEO analytics tracking — impressions,
// clicks, avg_position, category — unrelated purpose/schema). The original
// CREATE TABLE IF NOT EXISTS "seo_pages" migration silently no-op'd against
// it instead of creating this table, so every insert failed with "column
// does not exist" until this rename.
