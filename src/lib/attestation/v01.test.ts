...
import { parseJws, verifyAttestationJws, validateAttestation, canonicalize } from './v01';
...
describe('AuthiChain Attestation Contract v0.1', () => {
  ...
  it('rejects unsupported JWS algorithms and types before verification', async () => {
    ...
    const header = Buffer.from(JSON.stringify({ ...parsed.protected, alg: 'HS256' })).toString('base64url');
    const candidate = `${header}.${validJws.split('.')[1]}.${validJws.split('.')[2]}`;
    await expect(verifyAttestationJws(candidate, jwks.keys[0])).rejects.toThrow(/unsupported attestation JWS header/);
  });

  it('requires issuer identity fields', () => {
    expect(() => validateAttestation({ ...fixture, issuer: { id: 'https://authichain.com' } })).toThrow(/issuer.name/);
    expect(() => validateAttestation({ ...fixture, issuer: { name: 'AuthiChain' } })).toThrow(/issuer.id/);
  });
  ...
  it('rejects malformed or missing product identifiers', () => {
    expect(() => validateAttestation({ ...fixture, subject: { ...fixture.subject, gtin: 'abc' } })).toThrow(/gtin/);
    expect(() => validateAttestation({ ...fixture, subject: { ...fixture.subject, gtin: '1234567' } })).toThrow(/gtin/);
    expect(() => validateAttestation({ ...fixture, subject: { ...fixture.subject, serial: 123 } })).toThrow(/serial/);
    expect(() => validateAttestation({ ...fixture, subject: { ...fixture.subject, lot: 123 } })).toThrow(/lot/);
  });
  ...
  it('rejects missing, malformed, or empty evidence entries', () => {
    expect(() => validateAttestation({ ...fixture, evidence: null })).toThrow(/evidence must be an array/);
    expect(() => validateAttestation({ ...fixture, evidence: [{}] })).toThrow(/evidence\[0\]\.id/);
    expect(() => validateAttestation({
      ...fixture,
  ...
  it('rejects an expiry at or before issuance', () => {
    ...
    expect(() => validateAttestation({ ...fixture, expires_at: 'not-a-date' })).toThrow(/expires_at/);
  });
  ...
  it('allows explicit warning, blocked, revoked, and unknown states to remain inspectable', () => {
    for (const decision of ['warning', 'blocked'] as const) {
      expect(validateAttestation({ ...fixture, decision }).decision).toBe(decision);
    }
