...
export default function WatchdogSimulator() {
  ...
  return (
    <div className="bg-zinc-950 border border-zinc-900 rounded-xl p-6 hover:border-gold/30 transition-all shadow-2xl">
      ...
      <div className="space-y-6 mb-6">
        <div>
          <label className="flex justify-between text-sm text-zinc-400 mb-2">
            ...
            <span
              className={
                temperature > 75 ? "text-red-400 font-bold" : "text-zinc-100"
              }
            >
              {temperature}°F
            </span>
        ...
        <div>
          <label className="flex justify-between text-sm text-zinc-400 mb-2">
            ...
            <span
              className={
                humidity > 60 ? "text-red-400 font-bold" : "text-zinc-100"
              }
            >
              {humidity}%
            </span>
      ...
      {result && (
        <div
          className={`mt-6 p-4 rounded border ${result.compliant ? "bg-emerald-950/30 border-emerald-900/50" : "bg-red-950/30 border-red-900/50"}`}
        >
          <div className="flex items-center gap-2 mb-2">
            ...
            <span
              className={`font-bold ${result.compliant ? "text-emerald-400" : "text-red-400"}`}
            >
              {result.compliant
                ? "CRA COMPLIANT"
