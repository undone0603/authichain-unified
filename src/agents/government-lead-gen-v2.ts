...
//   • Weighted 5-agent consensus (Guardian / Archivist / Sentinel / Scout / Arbiter)
//   • QRON / outreach / pilot-mint record (chain mint delegated to scripts/mint-govchain-nfts.ts)
//
// DRY_RUN behavior:
//   process.env.DRY_RUN !== 'false'  →  no emails sent, no NFTs minted, no chain writes
//   process.env.DRY_RUN === 'false'  →  real outreach + mint record (chain still needs ALCHEMY + wallet + contract)
...
async function embedText(text: string): Promise<number[] | null> {
  const token = process.env.HF_TOKEN_PRIMARY || process.env.HUGGINGFACE_API_KEY;
...
function hitScore(haystack: string, terms: string[]): number {
...
async function runFiveAgentConsensus(lead: any): Promise<ConsensusResult> {
  ...
  const archivist = Math.min(
    100,
    40 +
      (lead.id ? 20 : 0) +
      (lead.awardsContext?.length ? 20 : 0) +
      (lead.spendingContext ? 20 : 0)
  );
  const sentinel = hitScore(haystack, [
    'buy american',
    'dfars',
    'traceability',
    'supply chain',
  ...
  const scout = Math.min(
    100,
    30 +
      (lead.agency && lead.agency !== 'Federal Agency' ? 30 : 0) +
      (lead.contact && lead.contact !== 'Procurement Team' ? 20 : 0) +
      (typeof lead.relevanceScore === 'number' ? Math.round(lead.relevanceScore * 0.2) : 10)
  );
  const weighted = Math.round(
    guardian * 0.35 + archivist * 0.2 + sentinel * 0.25 + scout * 0.08
  );
  const arbiter = Math.min(100, weighted + 12);
  const score = Math.min(
    100,
    Math.round(guardian * 0.35 + archivist * 0.2 + sentinel * 0.25 + scout * 0.08 + arbiter * 0.12)
  );

  return {
    approved: score >= 60,
    score,
    rationale: `5-agent: G${guardian} A${archivist} S${sentinel} Sc${scout} Ar${arbiter}`,
...
async function sendOutreach(lead: any, proposal: string, _qron: QRON): Promise<void> {
  ...
  console.log(
    `[gov-engine] Outreach ${result.ok ? 'sent' : 'failed'} to ${contactEmail} via ${result.provider}`
  );
}
...
async function mintPilotNFT(args: {
  ...
  if (DRY_RUN) {
    console.log(
      `[gov-engine] DRY_RUN mint skipped — would record pilot for ${args.agency} (lead ${args.leadId}, score ${args.consensusScore})`
    );
    return;
  }
  ...
  const supabase = supabaseAdmin();
  if (supabase) {
    const { error } = await supabase.from('gov_proposals').upsert(
      {
        notice_id: args.leadId,
        agency: args.agency,
        fit_score: args.consensusScore,
        status: 'draft',
        nft_metadata: {
          qronSignature: args.qronSignature,
          signer: ACTIVE_ENTITY.wallet,
        },
      },
      { onConflict: 'notice_id' }
    );
    if (error) {
      console.warn('[gov-engine] pilot upsert skipped:', error.message);
    } else {
      console.log(`[gov-engine] Pilot row recorded for ${args.agency} (${args.leadId})`);
    }
  }

  const hasChainConfig = !!(
    process.env.ALCHEMY_API_KEY &&
    (process.env.WALLET_PRIVATE_KEY || process.env.POLYGON_PRIVATE_KEY) &&
    (process.env.CONTRACT_ADDRESS || process.env.GOVCHAIN_NFT_CONTRACT)
  );
  if (!hasChainConfig) {
    console.warn(
      '[gov-engine] Chain mint deferred — set ALCHEMY_API_KEY + WALLET_PRIVATE_KEY + CONTRACT_ADDRESS and run scripts/mint-govchain-nfts.ts'
    );
  }
}
...
export async function runAdvancedGovernmentLeadGen() {
  console.log(
    `GovChain autonomous lead-gen v2.6 starting (DRY_RUN=${DRY_RUN}) — SAM Opps + Contract Awards v1 + USAspending + pgvector`
  );
  ...
  for (const lead of allLeads) {
    ...
    console.log(`[gov-engine] Found: ${lead.title} (${lead.agency}) ${consensus.rationale}`);
