...
async function loadPrivateKey(env?: AttestationEnv) {
  ...
  return { privateKey, kid: resolved.key.kid, publicJwk: resolved.key.publicJwk };
}
...
>(app: Hono<{ Bindings: E; Variables: V }>): void {
  app.post("/api/v1/attestation", async (c) => {
    try {
      ...
      const message = error instanceof Error ? error.message : "invalid attestation";
  ...
  app.post("/api/v1/attestation/verify", async (c) => {
    try {
      ...
      const attestation = await verifyAttestationJws(jws, resolved.key.publicJwk);
